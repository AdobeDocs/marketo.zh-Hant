...
1. 確保電子郵件的傳遞能力
   ...
   1. **設定SPF和DKIM。** 這些技術可確保您的收件者確信您的行銷人員電子郵件並非垃圾郵件。為協助防止收件者的垃圾郵件篩選器拒絕您的Marketo電子郵件，請依照下列步驟執行[設定SPF和DKIM以取得您的電子郵件傳遞能力](/help/marketo/product-docs/email-marketing/deliverability/set-up-spf-and-dkim-for-your-email-deliverability.md)。
...
如果您使用Google Apps代管公司電子郵件，將無法在您的網域下建立ubuse@或postmaster@電子郵件。 為瞭解決這個問題，您需要建立名為「濫用」和「郵遞員」的組。 屬於這些群組成員的使用者將會收到傳送至這些位址的電子郵件(例如postmaster@domain.com)。 有關建立組的詳細說明，請參閱[此處](https://support.google.com/a/answer/33343#adminconsole)。
...
1. 提交著陸頁面和電子郵件範本的URL
   ...
   對於CNAME，輸入您在步驟3(`[LandingPageCNAME]`)中選取的著陸頁面CNAME。 提交您的請求。
...
## 9.新增追蹤代碼至您的網站{#add-tracking-code-to-your-website}
