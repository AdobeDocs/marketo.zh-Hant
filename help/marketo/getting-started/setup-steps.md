...
* 一些基本帳戶設定
* 將登錄頁URL和電子郵件連結貼上品牌，以提高信任度和可交付性
* 正在同步您的CRM
* 向公司網站添加跟蹤代碼
...
>[!TIP]
>
>如果 [打印出核對表](/help/marketo/getting-started/setup-steps/setup-checklist.md){target=&quot;_blank&quot;}，您可以在完成項目時將其簽出。

## 登錄並建立其他Marketo用戶 {#log-in-and-create-additional-marketo-users}

1. 登錄Marketo [這裡](https://app.marketo.com/){target=&quot;_blank&quot;}，使用您通過電子郵件收到的憑據。

   ![](assets/new-login-screen-hand.jpg)

恭喜您！你現在在Marketo，可以開始探索了。 您可能希望邀請營銷團隊中的同事加入您。 您可以通過添加新用戶來執行此操作。

轉到 **管理** 的子菜單。

>[!TIP]
>
>在此時，您可以按一下 **我的帳戶** 更改帳戶和位置設定，並設定新訂閱名稱。
...
（可選）使用日曆選取器輸入邀請的原因和訪問到期日期。 按一下 **確定**。
...
>[!TIP]
>
>到期日對短期外部利益相關方或咨詢人非常有利，他們只需在很短的時間內訪問Marketo。

>[!NOTE]
>
>到期日期到達時，用戶將收到到期通知，並且帳戶已鎖定。

選擇角色並按一下 **下一個**。 標準用戶有權訪問除管理員之外的所有區域。
...
新用戶現在列在「用戶」頁籤中，應該收到一封電子郵件，其中包含建立密碼和登錄的連結。 下一步！
...
要檢索要用登錄頁CNAME替換的帳戶字串，請轉到「管理」區域。
...
這是 `[AccountString]`。 保存它。 您需要在步驟5中將其交給IT部門。
...
跟蹤連結的形式為： `mkto-[a-z][4 digits].com`。
...
`1)` 對於登錄頁，添加DNS條目(CNAME) **[登錄頁名稱]**。**[公司域]**.com，指向 **[帳戶字串]**.mktoweb.com。
...
在登錄頁的域名欄位中輸入新域名。 這應該是：

`[LandingPageCNAME].[CompanyDomain].com`

![](assets/image2015-1-6-14-3a3-3a6.png)

在「回退」頁欄位中，輸入登錄頁不可用時希望用戶轉到的URL。 如果沒有備用頁，則可以使用公司首頁。 在「首頁」欄位中，輸入公司網站。

![](assets/image2015-1-6-14-3a2-3a46.png)

在「管理」區域，選擇「電子郵件」以添加電子郵件CNAME
...
根據您公司使用的CRM，從以下選項中進行選擇。
    ...
    * [整合Marketo與Microsoft動力](/help/marketo/product-docs/crm-sync/microsoft-dynamics-sync/understanding-the-microsoft-dynamics-sync.md)
...
>[!NOTE]
>
>您是Launch Pack客戶嗎？ 可以跳過此步驟。 您的顧問將在IT設定說明文檔中為您提供Munchkin代碼說明。

Marketo有自定義跟蹤JavaScript（稱為Munchkin），您可以使用它跟蹤任何網頁上的人員活動。 Munchkin需要將您的網站整合到Marketo。 按照以下步驟執行 [將Munchkin跟蹤代碼添加到您的網站](/help/marketo/product-docs/administration/additional-integrations/add-munchkin-tracking-code-to-your-website.md){target=&quot;_blank&quot;}。

>[!NOTE]
>
>添加跟蹤代碼所需的HTML體驗。

所有設定步驟都結束。 只剩下一樣東西，就是潛進去，用Marketo!
