...
1. 尋找並選取`{{SP_Send_Alert_Info}}` **[!UICONTROL Token]**&#x200B;並按一下&#x200B;**[!UICONTROL Insert]**。
   ...
   >[!NOTE]
   >
   >`{{SP_Send_Alert_Info}}`是警示電子郵件的特殊權杖。 請參閱[使用傳送警示資訊Token](/help/marketo/product-docs/email-marketing/general/using-tokens/use-the-send-alert-info-token.md){target="_blank"}瞭解更多資訊。
