---
unique-page-id: 10100636
description: 新增應用程式內訊息影像 — Marketo檔案 — 產品檔案
title: 新增應用程式內訊息影像
exl-id: b4617338-04bc-4235-bff1-7ec89a9b2c6a
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '337'
ht-degree: 0%

---

# 新增應用程式內訊息影像 {#add-in-app-message-images}

您可以在此處選擇和自訂應用程式內訊息影像。

1. 選取應用程式內訊息，然後按一下 **編輯草稿**.

   ![](assets/image2016-5-4-10-3a20-3a14.png)

1. 在「佈局」頁簽上，從六個模板中選擇一個。 有三個彈出窗口和三個全螢幕選項。

   >[!TIP]
   >
   >範本之間有何差異？ 請參閱 [選擇應用程式內訊息的版面](/help/marketo/product-docs/mobile-marketing/in-app-messages/creating-in-app-messages/choose-a-layout-for-your-in-app-message.md) 開始之前。

   ![](assets/image2016-5-4-10-3a21-3a33.png)

1. 按一下 **樣式** 標籤，然後按一下範本的影像區域。

   ![](assets/image2016-5-3-16-3a53-3a23.png)

1. 按一下 **選擇影像**.

   ![](assets/image2016-5-6-8-3a53-3a55.png)

1. 從Design Studio中選取影像，然後按一下 **選擇**.

   ![](assets/image2016-5-6-8-3a58-3a40.png)

   >[!NOTE]
   >
   >影像大小限制為1440 x 2560像素和5MB。

1. 改變主意，想使用不同的影像嗎？ 沒問題。 按一下 **X** 在影像檔案名稱旁。

   ![](assets/image2016-5-6-9-3a0-3a16.png)

1. 按一下 **移除**. 現在，你可以選另一個。

   ![](assets/image2016-5-6-9-3a1-3a3.png)

1. 在「影像屬性」中，選取按照「高度」、「寬度」或「兩者」調整影像的按鈕，以適當調整影像大小。

   ![](assets/image2016-5-6-9-3a4-3a47.png)

1. （可選）應用影像邊框。 預設設定為 **關閉**. 首先，按一下顏色，或在檢色器上輸入十六進位或RGB號來選取顏色。

   ![](assets/image2016-5-6-9-3a9-3a0.png)

1. 按一下箭頭可更改邊框寬度（像素）。 你會在影像上看到它變化。

   ![](assets/image2016-5-6-9-3a35-3a43.png)

1. 使用滑桿選取圓角半徑。 從左到右選擇位置：0、4、8、12或16像素。 預設值為8像素（中心）選區。

   ![](assets/image2016-5-6-9-3a39-3a28.png)

1. 選擇邊距（開啟或關閉）。 **開啟** 為預設設定。

   ![](assets/image2016-5-6-9-3a42-3a15.png)

1. 核取方塊以定義影像點選動作。

   ![](assets/image2016-5-6-9-3a48-3a58.png)

1. 每個平台都有個別的動作（請參閱附註）。

   ![](assets/image2016-5-6-9-3a50-3a15.png)

1. 按一下每個下拉式清單以顯示選項。 選取一個。

   ![](assets/image2016-5-6-9-3a52-3a41.png)

   >[!NOTE]
   >
   >對於影像、按鈕或背景的點選動作，您可以為Apple和Android平台設定不同的動作。 例如，Apple和Android對深層連結的處理方式不同。 如果您的訊息只會傳送至一個平台，請將另一個平台保留為預設設定，或選取 **無**.

幹得好！ 現在，是時候 [建立應用程式內訊息的文字](/help/marketo/product-docs/mobile-marketing/in-app-messages/creating-in-app-messages/create-in-app-message-text.md).

>[!MORELIKETHIS]
>
>* [了解應用程式內訊息](/help/marketo/product-docs/mobile-marketing/in-app-messages/understanding-in-app-messages.md)
>* [選擇應用程式內訊息的版面](/help/marketo/product-docs/mobile-marketing/in-app-messages/creating-in-app-messages/choose-a-layout-for-your-in-app-message.md)

