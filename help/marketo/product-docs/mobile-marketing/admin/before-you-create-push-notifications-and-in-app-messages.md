...
針對應用程式內傳訊，顯示觸發程式預設為&#x200B;**應用程式開啟**。 如果您想要使用任何自訂事件來觸發應用程式內訊息的顯示（例如，**按一下[加入購物車]**、**檢視設定頁面**），請建立所需事件的清單，並將清單提供給您的行動應用程式開發人員。 開發人員會將自訂事件新增至程式碼中。 在核准後，在設定您的對象時，它們會顯示為顯示觸發器。 **警告**：自訂事件編碼核准程式可能需要一些時間才能完成。
