---
unique-page-id: 7512432
description: 設定行動應用程式Android推播存取 — Marketo檔案 — 產品檔案
title: 設定行動應用程式Android推播存取
exl-id: 1204f36a-d960-4af6-a5d0-ee6b6720026a
feature: Mobile Marketing
source-git-commit: 431bd258f9a68bbb9df7acf043085578d3d91b1f
workflow-type: tm+mt
source-wordcount: '102'
ht-degree: 0%

---

# 設定行動應用程式Android推播存取 {#configure-mobile-app-android-push-access}

1. 按一下 **管理員**.

   ![](assets/image2015-4-22-16-3a12-3a32.png)

1. 選取 **行動應用程式**.

   ![](assets/image2015-4-22-16-3a14-3a29.png)

1. 選取所需的行動應用程式。

   ![](assets/image2015-4-22-16-3a33-3a19.png)

1. 下 **推播存取型別**，選取 **Android** 並按一下 **設定。**

   ![](assets/image2016-6-15-15-3a16-3a22.png)

   >[!NOTE]
   >
   >您將需要 **伺服器API金鑰** 和 **專案編號** 來自您的行動應用程式開發人員。 開發人員會透過登入Google Play開發人員控制檯來接收這些訊息，以註冊您的應用程式並啟用雲端訊息。

1. 輸入您的伺服器API金鑰和專案編號。 按一下 **儲存**.

   ![](assets/image2015-4-22-18-3a54-3a54.png)

   真貼心。 請務必使用iOS設定應用程式。

>[!MORELIKETHIS]
>
>[設定行動應用程式iOS推送存取權](/help/marketo/product-docs/mobile-marketing/admin/configure-mobile-app-ios-push-access.md)
