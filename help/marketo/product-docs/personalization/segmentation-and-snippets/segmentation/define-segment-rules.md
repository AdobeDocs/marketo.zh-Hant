...
1. 按一下 **智慧清單** 並新增篩選器。
   ...
   >[!CAUTION]
   >
   >目前不支援的區段 _過去_ 和 _在時間範圍內_  篩選器上的運運算元。 這是因為區段只有在記錄變更資料值時才會檢查更新。 這些值為 _not_ 已記錄會自動變更的專案，例如公式欄位和日期。 此外，不支援具有相對日期範圍的日期運運算元，因為這些運運算元是在細分核準時計算的，而不是在「變更資料值」活動時計算的。
...
1. 為篩選器填入適當的值。
   ...
   >[!CAUTION]
   >
   >帳戶欄位的活動記錄行為可能會影響資格。 因此，我們建議您在定義節段規則時，不要使用「科目」欄位。
...
1. 請檢視儀表板，以快速概略瞭解圓形圖中的區段以及套用的規則。

   ![](assets/image2014-9-15-11-3a36-3a19.png)

幹得好！ 這些區段在Marketo的許多地方都會派上用場。

>[!NOTE]
>
>個人可能符合不同區段的資格，但最終僅屬於依賴以下專案的區段： [區段的優先順序](/help/marketo/product-docs/personalization/segmentation-and-snippets/segmentation/segmentation-order-priority.md).
