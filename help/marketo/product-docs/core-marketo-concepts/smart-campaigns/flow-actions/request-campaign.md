---
unique-page-id: 2951703
description: 瞭解如何使用請求行銷活動流程步驟。 有人達到此步驟時，觸發另一個要執行的行銷活動。
title: 請求行銷活動
exl-id: 5deecb56-c25f-4e3b-bd85-f9cac1c7149e
feature: Smart Campaigns
source-git-commit: 60c5603fa29bb1039b9d477633beb2c6f5c63486
workflow-type: tm+mt
source-wordcount: '115'
ht-degree: 4%

---

# 請求行銷活動 {#request-campaign}

您可以使用&quot;[!UICONTROL Request Campaign]&quot;單一流程步驟，將人員放入特定行銷活動。

>[!NOTE]
>
>您要將人員放置到的行銷活動必須處於作用中狀態，且有[已要求行銷活動](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/using-smart-campaigns/setting-up-a-trigger-smart-campaign-for-sales-using-campaign-is-requested.md){target="_blank"}觸發器。

1. 在&#x200B;**[!UICONTROL Database]**&#x200B;中，尋找並選取想要的人員。

   ![](assets/request-campaign-1.png)

1. 按一下「**[!UICONTROL Person Actions]**」，將游標暫留在「**[!UICONTROL Special]**」上，然後選取「**[!UICONTROL Request Campaign]**」。

   ![](assets/request-campaign-2.png)

1. 選取您希望人員進入的行銷活動，然後按一下&#x200B;**[!UICONTROL Run Now]**。

   ![](assets/request-campaign-3.png)

1. 在畫面的右上角，您將會看到操作完成的時間。 按一下「**[!UICONTROL View Results]**」。

   ![](assets/request-campaign-4.png)

   您的人員目前處於請求的行銷活動中。

   ![](assets/request-campaign-5.png)

>[!TIP]
>
>使用&quot;[!UICONTROL Campaign is Requested]&quot;觸發器或篩選器來尋找受此流程步驟影響的人員。
