---
unique-page-id: 1146956
description: 有趣的時刻——行銷人員檔案——產品檔案
title: 有趣的時刻
translation-type: tm+mt
source-git-commit: efadb7eb3845012c273e1a60f9cd98ac884eb543
workflow-type: tm+mt
source-wordcount: '106'
ht-degree: 0%

---


# 有趣的時刻{#interesting-moment}

## 概述{#overview}

如果您有[Marketo Sales Insight](http://docs.marketo.com/display/docs/assets/sales-insight.php)，則可使用&#x200B;**Interest Moment**&#x200B;流程步驟，讓銷售團隊能夠洞悉您的員工正在做的酷炫事。

![](assets/image2016-1-27-11-3a0-3a21.png)

## 用法{#usage}

1. 選擇您要使用的有趣時刻&#x200B;**type**。

   ![](assets/image2014-9-22-10-3a49-3a51.png)

1. 建立&#x200B;**description**，以說明銷售團隊的有趣時刻。

   ![](assets/image2014-9-22-10-3a49-3a55.png)

   >[!TIP]
   >
   >**少就是多**。與您的銷售團隊合作，確保有趣的時刻真的很有趣。

   [有趣時刻的預付碼](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/tokens-for-interesting-moments.md)

   >[!NOTE]
   >
   >**相關文章**
   >
   >    
   >    
   >    * [使用有趣的瞬間](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/using-interesting-moments.md)
   >    * [有趣時刻的Token](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/tokens-for-interesting-moments.md)


您也可以用來製作真正有用的動態說明。