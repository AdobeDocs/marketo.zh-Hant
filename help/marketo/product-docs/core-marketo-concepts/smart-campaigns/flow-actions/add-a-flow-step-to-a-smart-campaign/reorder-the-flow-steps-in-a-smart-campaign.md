---
unique-page-id: 1146985
description: 重新排序智慧行銷活動中的流程步驟 — Marketo檔案 — 產品檔案
title: 重新排序智慧促銷活動中的流程步驟
exl-id: 5cf4dac1-d21a-44ff-a485-c24a91e83fc9
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '96'
ht-degree: 0%

---

# 重新排序智慧促銷活動中的流程步驟 {#reorder-the-flow-steps-in-a-smart-campaign}

流程步驟從上到下執行。 以下說明如何重新排序流程步驟，以符合您的需求。

>[!PREREQUISITES]
>
>[新增流量步驟至智慧型促銷活動](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/flow-actions/add-a-flow-step-to-a-smart-campaign.md)

1. 在智慧型促銷活動流量標籤中，將流量步驟拖放至您想要的位置。

![](assets/image2014-9-22-13-3a49-3a11.png)

>[!NOTE]
>
>流程步驟按照它們在流中的顯示順序運行。

有些事情很簡單。 把拖拽走！
