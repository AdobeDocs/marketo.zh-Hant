---
unique-page-id: 2953331
description: 瞭解如何從方案排程檢視重新執行Smart Campaign。 依需求再次執行批次行銷活動。
title: 在方案排程視圖中重新執行智慧行銷活動
exl-id: 59ef61f5-cd7d-440e-afcf-54a79d1225ae
feature: Programs, Smart Campaigns
source-git-commit: d5258342dd89a8f46a9897e9c7ee8dad4a33df59
workflow-type: tm+mt
source-wordcount: '136'
ht-degree: 14%

---

# 在方案排程視圖中重新執行智慧行銷活動 {#rerun-a-smart-campaign-in-the-program-schedule-view}

可以直接從方案排程檢視建立現有Smart Campaign的新執行。

1. 前往 **[!UICONTROL Marketing Activities]**。

   ![](assets/login-marketing-activities-3.png)

1. 選取包含您的Smart Campaign的方案。

   ![](assets/image2015-4-16-14-3a40-3a11.png)

1. 在排程檢視中，按一下您想要設定新回合的日期，並為您的專案提供容易理解的名稱（例如「第二次邀請」）。

   ![](assets/image2015-4-16-14-3a42-3a0.png)

1. 選取專案型別功能表下拉式清單，然後選取要重新執行的Smart Campaign。

   ![](assets/image2015-4-16-15-3a26-3a33.png)

   >[!TIP]
   >
   >您也可以從[方案焦點](/help/marketo/product-docs/core-marketo-concepts/marketing-calendar/understanding-the-calendar/understand-enable-program-focus.md)進行此動作。

此Smart Campaign現在已排程進行另一個執行。 如果其中包含傳送電子郵件步驟，這些步驟也會顯示在這裡。
