...
此範例為具有註冊頁面的活動計畫、三封邀請電子郵件，以及利用Marketo Engage活動計畫的後續電子郵件。 適合您需要註冊的所有活動，包括路演、午餐、晚宴或展銷活動簡報。 使用Marketo Engage事件應用程式，在iPad上進行簽到和出勤追蹤。
...
>[!NOTE]
>
>在活動開始前，請將註冊資訊下載至您的iPad。 接著，將訪客簽入，並將出席資訊同步回Marketo Engage。 如果您沒有平板電腦，可以 [匯入出席者](/help/marketo/product-docs/core-marketo-concepts/programs/working-with-programs/import-members-from-a-spreadsheet-into-a-program.md){target="_blank"} 之後，在方案的「成員」標籤下，再傳送您的後續追蹤電子郵件。
