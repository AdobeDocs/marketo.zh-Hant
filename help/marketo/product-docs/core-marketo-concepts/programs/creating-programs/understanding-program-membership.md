...
>[!NOTE]
>
>Marketo現在已將所有訂閱的語言標準化，因此您可能會在訂閱中看到銷售機會/銷售機會，並在我們的檔案中看到相關人員。 這些術語的意思相同；它不會影響文章指示。 此外，也有一些其他變更。 [深入了解](/help/marketo/product-docs/crm-sync/salesforce-sync/understanding-the-salesforce-sync.md).
...
1. 個人填寫 [登陸頁面上的表單](/help/marketo/getting-started/quick-wins/landing-page-with-a-form.md) 在程式中。
...
1. 個人註冊或出席 [與活動計畫同步的網路研討會](/help/marketo/product-docs/demand-generation/events/understanding-events/event-partners.md).
...
1. 新增個人至SFDC促銷活動，此促銷活動為 [已同步至程式](/help/marketo/product-docs/crm-sync/salesforce-sync/sfdc-sync-details/sfdc-sync-campaign-sync.md).
...
計畫狀態是人們在計畫中執行的步驟（例如，已邀請、已回覆、已出席、無節目）。 這些步驟的定義方式為 [頻道](/help/marketo/product-docs/administration/tags/create-a-program-channel.md).

![](assets/image2015-2-5-15-3a14-3a48.png)

>[!NOTE]
>
>個人無法後移至先前的計畫狀態。 狀態進度是唯一方式。

## 成功狀態 {#success-statuses}

方案的目的是建立與個人或潛在客戶有意義的互動。 成功是在個人達到達到該目標的狀態時標示。

>[!NOTE]
>
>對於網路研討會而言，如果註冊實際上並未觀看網路研討會，則其意義不大。 在此情況下，出席是成功的。

## 贏取方案  {#acquisition-program}

當新名稱進入系統作為計畫成員時，Marketo會自動將該計畫設為「贏取」。 這會建立以下專案的評分： [首次接觸歸因](/help/marketo/product-docs/reporting/revenue-cycle-analytics/revenue-tools/attribution/understanding-attribution.md).
