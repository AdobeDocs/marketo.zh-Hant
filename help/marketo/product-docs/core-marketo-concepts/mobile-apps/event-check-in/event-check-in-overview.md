---
description: 事件簽入總覽
title: 事件簽入總覽
exl-id: aae09cd8-5f8f-48c7-9d26-63c8345445d0
feature: Mobile Marketing
source-git-commit: 0abb315be0f9cb5f42fa41d72b446de8c2f62c1e
workflow-type: tm+mt
source-wordcount: '280'
ht-degree: 0%

---

# 事件簽入總覽 {#event-check-in-overview}

>[!IMPORTANT]
>
>2023年10月2日，Adobe從所有應用程式商店移除Marketo事件應用程式。 如果您的平板電腦/行動裝置上已安裝應用程式，目前可繼續使用。 一旦您的Marketo Engage執行個體移轉至Adobe身分以驗證Marketo，您將無法再存取應用程式。 [深入了解](https://nation.marketo.com/t5/product-discussions/marketo-events-app-and-marketo-moments-app-end-of-life/m-p/340712/highlight/true#M193869){target="_blank"}.

適用於的Marketo Events應用程式 [!DNL iPad] 或 [!DNL Android] 平板電腦可讓您在人員到達您的活動時輕鬆立即簽到。

本文會概述完整程式。 標題會連結至個別文章，而某些Marketo活動會連結至此大綱。

有些活動會發生在事件之前，例如建立新角色和邀請出席者。 有些會在活動期間發生，例如登記出席者。 在事件後，您會將資料與Marketo同步，並更新人員的狀態。

祝您活動順利！

## 準備您的活動

在事件之前：

* [建立新的Marketo角色](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/event-check-in/grant-users-access-to-the-check-in-app.md){target="_blank"} 的 [!DNL iPad] 或 [!DNL Android] 事件的使用者
* [邀請新的Tablet使用者參加此活動](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/event-check-in/grant-users-access-to-the-check-in-app.md){target="_blank"} 並將新角色指派給他們
* [建立新的Marketo活動程式](/help/marketo/product-docs/demand-generation/events/understanding-events/create-a-new-event-program.md){target="_blank"}
* 指定事件日期
* 將人員新增至您的活動

## 設定 [!DNL iPad] 或 [!DNL Android] 平板電腦

若要取得 [!DNL iPad] 或 [!DNL Android] 平板電腦就緒：

* 下載並安裝Marketo事件 [!DNL iPad] 或 [!DNL Android] 平板電腦應用程式
* 確認登入認證
* 同步應用程式以填入事件
* 確認簽入運作中

## 使用簽入人員 [!DNL iPad] 或平板電腦

在活動上：

* 登入應用程式
* 尋找來賓（並建立新人員）
* [簽入來賓](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/event-check-in/check-people-into-your-event-from-your-tablet.md){target="_blank"} （反向簽入，簽入時編輯個人記錄）
* 同步 [!DNL iPad] 或 [!DNL Android] 使用Marketo的平板電腦應用程式
