---
description: 瞭解將您的Smart Campaigns視覺化的Engagement Map。 用它在Marketo中直覺地檢視觸發器、篩選器和流程步驟。
title: 參與地圖概觀
...
否。 Engagement Map具有完整的回溯相容性。 只要按一下按鈕，您就可以在視覺化參與對映介面中檢視每個現有的行銷活動。

**在Marketo Engage中擁有參與地圖是否表示我將會失去資料夾結構的存取權，並且必須重新學習如何建立Smart Campaign？**

否。 參與地圖提供兩個領域的最佳值；它是目前您建立行銷活動的方式的補充。 所有使用者仍可獲得Marketo Engage的強大功能集，現在提供清晰的視覺效果作為補充。

**參與地圖是否為付費附加元件？**

否。 所有訂閱下的所有現有和新Marketo Engage使用者都可存取此免費功能。

**身為Marketo Engage管理員，我是否應該負責開啟/關閉「參與對應」？**

否。 參與地圖將可供其執行個體中的所有使用者使用。 您無法為某些使用者、工作區等開啟或關閉此功能。
...
* 功能：所有視覺效果均已開箱即用，讓使用更輕鬆；巢狀結構可讓您深入探索並檢視全貌。
