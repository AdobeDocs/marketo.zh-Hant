...
description: 如何新增連結欄位，以將自訂物件連線至人員或公司，以進行一對多和多對多結構，包括使用中介物件。
...
>[!IMPORTANT]
>
>在多對多關係中，Marketo Engage僅支援每個bridge物件的單一edge物件。 在下方提供的範例中，每個註冊只能連結至單一課程。 不過，每個Edge物件可以有許多橋接物件，正如每個課程都有許多學生註冊（多對一關係）。 如果您的自訂物件資料結構化，使得每個Edge物件記錄（一對多或多對多）都有一個以上的Bridge物件記錄，您可以建立多個Bridge物件記錄，讓每個記錄都參照單一Edge物件記錄，以便在Marketo中表示該資料。

## 建立一對多結構的連結欄位 {#create-a-link-field-for-a-one-to-many-structure}

請依照下列步驟，為一對多結構在自訂物件中建立連結欄位。
...
1. 為連結欄位命名並新增選用的[!UICONTROL Description]。 選取[!UICONTROL Link]資料型別。

   ![](assets/add-marketo-custom-object-link-fields-5.png)

   >[!CAUTION]
   >
   >自訂物件獲得核准後，就無法返回上一步來建立、編輯或刪除[!UICONTROL Link]或[!UICONTROL Dedupe Field]。
...
1. 建立兩個連結欄位，一次一個。 首先，為資料庫清單成員的欄位命名（例如leadID）。 新增選用的[!UICONTROL Description]。 選取[!UICONTROL link] [!UICONTROL Data Type]。
...
下一步是在您的智慧行銷活動中，於篩選器中使用這些自訂物件。 透過多對多關係，您可以選取多個人員或公司以及多個自訂物件。 在下列範例中，資料庫中符合這些條件的任何人都將會列出。 coursename欄位來自課程自訂物件，而註冊等級來自中繼物件。
