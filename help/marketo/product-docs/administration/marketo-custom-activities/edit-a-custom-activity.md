---
unique-page-id: 10100269
description: 瞭解如何編輯自訂活動，以及草稿或發佈狀態如何受到影響。
title: 編輯自訂活動
exl-id: bdfbf0aa-0035-46d2-80dd-21a1366c6514
feature: Custom Activities
TQID: https://experienceleague.adobe.com/ilHeQJ-3pI0BCr2TLJuFVpHDfscSdZbNnmy9jdlunms
product_v2: id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2: id: d1d0a9cd-295d-4976-8c39-ddae266f240e
topic_v2: id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 105
ht-degree: 12%

---

# 編輯自訂活動 {#edit-a-custom-activity}

若要變更已建立的自訂活動，請遵循下列步驟。

1. 前往「**[!UICONTROL Admin]**」區域。

   ![](assets/edit-a-custom-activity-1.png)

1. 按一下「**[!UICONTROL Marketo Custom Activities]**」。

   ![](assets/edit-a-custom-activity-2.png)

1. 選取您要編輯的自訂活動。

   ![](assets/edit-a-custom-activity-3.png)

1. 按一下&#x200B;**[!UICONTROL Custom Activity Actions]**&#x200B;並選取&#x200B;**[!UICONTROL Edit Activity]**。

   ![](assets/edit-a-custom-activity-4.png)

   「編輯活動型別」即會出現。 在此範例中，拼寫錯誤正在被更正。

   ![](assets/edit-a-custom-activity-5.png)

1. 輸入更新的資訊，然後按一下&#x200B;**[!UICONTROL Submit]**。

   ![](assets/edit-a-custom-activity-6.png)

   已更新自訂活動。

   >[!NOTE]
   >
   >如果您的活動在編輯時為草稿，則仍為草稿。 如果已發佈，則狀態會變更為已發佈並附帶草稿。
