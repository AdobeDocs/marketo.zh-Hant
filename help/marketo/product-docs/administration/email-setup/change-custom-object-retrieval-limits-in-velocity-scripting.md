...
如果您使用[!DNL Velocity Script]在電子郵件中顯示自訂物件資料，則此功能可能適用於您的使用案例。 依預設，您可以從Velocity指令碼存取10個父項自訂物件。 如果您需要存取更多資訊，請參閱下列步驟。
...
您可以參照直接連線至潛在客戶或連絡人的父項和子項自訂物件，但不能參照第三級自訂物件。 對於每個自訂物件，每個人員/聯絡人的10個最近更新記錄在執行階段可用，並且按照從最近更新（於0時）到最舊更新（於9時）的順序排列。
...
您現在可以從[!DNL Velocity script]存取更多自訂物件。
