...
>[!CAUTION]
>
>工作區的設定方式可能很複雜。 請連絡[Marketo支援](https://nation.marketo.com/t5/Support/ct-p/Support)，瞭解他們是否適合您。
...
>[!NOTE]
>
>瞭解如何[建立新的工作區](/help/marketo/product-docs/administration/workspaces-and-person-partitions/create-a-new-workspace.md)。

## 跨工作區共用 {#sharing-across-workspaces}

下列步驟說明如何在工作區之間共用資產。 無論您想要共用什麼，運作方式都一樣；此範例則展示分段。
...
1. 選取您想要共用該資料夾的工作區，然後按一下 **[!UICONTROL Save]**。 「共用資料夾」對話框只會顯示您擁有檢視權限的工作區。
   ...
   >[!NOTE]
   >
   >原始資料夾現在會有一個小型綠色箭頭，表示已共用該資料夾。 在已共用的工作區中，資料夾會出現表示唯讀的掛鎖。
...
對於非範本的資產，建議在程式內將其復製為本機資產。 具有適當的存取層級後，您即可將這些資產拖放至另一個工作區：
...
>[!NOTE]
>
>含有成員的程式不能從一個工作區移動到另一個工作區。

## 人員分割 {#person-partitions}

人員分割的功用類似於獨立的資料庫。 每個分割都具有自己的人員，不會去除重複或與其他分割混合。 如果您的業務使用案例需要具有相同電子郵件地址的重複記錄，請聯絡[Marketo支援](https://nation.marketo.com/t5/Support/ct-p/Support)。
