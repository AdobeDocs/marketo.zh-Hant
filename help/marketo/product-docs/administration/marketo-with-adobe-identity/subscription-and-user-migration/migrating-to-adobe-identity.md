...
當Adobe排程訂閱的使用者移轉時，Marketo Engage產品管理員將可存取移轉主控台，這可在整合主題下管理區域的導覽功能表中找到。
...
第一節　涉及訂閱內使用者的電子郵件驗證狀態。

第二部分說明訂閱內使用者的移轉和啟用(亦即移轉和享有Marketo Engage產品的權利)狀態。
...
管理員可在「移轉控制檯」的「使用者移轉」標籤及「Marketo管理員」區域的「使用者與角色」標籤中，或透過使用者的「帳戶設定」重新傳送驗證電子郵件。 就像使用者邀請電子郵件一樣，驗證電子郵件中的連結將在3天後過期。 如需電子郵件驗證的詳細資訊，請參閱社群 &lt;nation.marketo.com> 以及電子郵件驗證檔案&lt; https://experienceleague.adobe.com/docs/marketo/using/product-docs/administration/users-and-roles/email-verification.html?lang=en>。

重要

如果Marketo Engage使用者未驗證其電子郵件地址，則無法將他們移轉至Adobe ID，而且在移轉訂閱完成之後，將無法存取Marketo訂閱。 若要在使用者移轉完成後重新取得存取權，Marketo產品管理員需要將使用者新增為新使用者。
...
執行移轉的管理員需要按一下「完成移轉」按鈕，完成移轉確認。 系統會提示他們確認移轉已完成。
...
Adobe需要客戶在30天內完成自助移轉。 如果到期日已過，系統不會封鎖管理員移轉使用者或完成移轉，但管理員只能依需求移轉使用者。  如果管理員需要更多時間，可以延長訂閱的到期日。

熒幕擷圖

按一下「延長有效期」按鈕後，日期將於一週後更新。 管理員最多可將到期日延長三次。

熒幕擷圖

熒幕擷圖

當客戶在到期日前未完成移轉時，將依Adobe進行外聯。
...
如果管理員知道使用者無法/不會驗證其電子郵件或不應透過「略過移轉」按鈕移轉，則略過該使用者的移轉
...
使用者若要移轉至Adobe ID，必須經過電子郵件驗證。 如果有任何使用者尚未驗證其電子郵件地址且需要移轉，管理員可以觸發驗證電子郵件，以再次傳送給使用者。 選取未經驗證的使用者後，「驗證電子郵件」按鈕將會啟動，並提示您傳送電子郵件。

熒幕擷圖

一旦管理員按一下「驗證電子郵件」按鈕，他們將收到傳送電子郵件的通知。
...
若要略過使用者，管理員可以選取想要的使用者。 將會啟用「略過移轉」按鈕。 按一下「略過移轉」按鈕後，頁面將重新整理，且所選使用者的驗證狀態和移轉狀態將更新為「已略過」。

熒幕擷圖

如果確定需要移轉使用者，管理員可以取消略過先前略過的使用者。

若要取消略過使用者，管理員可以選取想要的使用者。 「取消略過移轉」按鈕將會啟動。 按一下「取消略過移轉」按鈕後，頁面將會重新整理。  所選使用者的驗證狀態將更新為其目前狀態（「已驗證」或「未驗證」），而使用者的移轉狀態將更新為「未開始」。
...
將Marketo使用者移轉至AdobeID

對於想要在使用者移轉程式期間加強控制的客戶，Marketo支援對擁有75名或更多使用者的訂閱採取自助服務方法。 Marketo產品管理員將能選取要批次移轉的使用者，或一次選取所有符合資格的使用者。 選取使用者後，管理員即可選擇「立即移轉」或「排程移轉」以供稍後使用，讓管理員在移轉使用者及移轉時間方面擁有最大的彈性和控制能力。 管理員在訂閱中也會看到「移轉所有使用者」的選項。

例如，管理員可以選取要先移轉的一組「高級」使用者。 一旦這些使用者移轉成功完成，他們就可以根據工作區、業務功能或角色等專案選取不同的群組，以透過進一步批次使用者移轉。 或者，他們可以在第一個批次成功後決定移轉訂閱中的所有其餘使用者。 目標是要提供最大的彈性來將AdobeID轉出給使用者。

所有使用者移轉同時發生，應該會在60秒內成功完成。 雖然特定使用者正在進行使用者移轉，但使用者最多可能會失去一分鐘的存取權，而且只有在使用者已登入應用程式的情況下才會發生這種情況。 完成使用者移轉後，使用者將收到一封電子郵件，說明如何使用Adobe身分登入Marketo Engage。 使用者需要透過電子郵件中的按鈕連結接受邀請。 接受邀請後，使用者應使用Adobe ID登入。 如需如何使用Adobe ID登入Marketo Engage的指示，請參閱此處。 &lt; https://experienceleague.adobe.com/docs/marketo/using/product-docs/administration/marketo-with-adobe-identity/user-sign-in-with-adobe-id.html>

熒幕擷圖

使用者移轉是獨立處理，因此如果使用者移轉失敗，Adobe會繼續處理其他使用者移轉。 如果發生使用者移轉失敗，管理員不需要採取任何動作。 Adobe工程部門將致力於儘快解決此問題。 管理員將收到有關錯誤的電子郵件通知，並提醒Adobe正在努力立即解決問題。 如果使用者的移轉失敗並且該使用者已登入Marketo Engage，則使用者最多可能損失2分鐘的存取權，同時會發生移轉重新嘗試。  如果使用者的移轉失敗，使用者可以繼續使用其Marketo身分存取Marketo Engage，直到他們收到電子郵件通知得知其移轉成功，並且受邀使用Adobe ID登入。

熒幕擷圖

立即移轉

管理員可以選取一或多個要依需求移轉的使用者。 這將會立即觸發使用者移轉。 若要移轉一或多個使用者，管理員會選取所需使用者，然後啟用「立即移轉」按鈕。
...
管理員可以選取一個或多個使用者，以排程在稍後日期進行移轉。 若要排程一或多個使用者的移轉，管理員會選取所要的使用者，並啟動「排程移轉」按鈕。

熒幕擷圖

注意

只有所有選取的使用者都具有「已驗證」驗證狀態時，「排程移轉」按鈕才會生效。

按一下「排程移轉」按鈕後，系統會提示管理員選取所選使用者的所需移轉日期。 管理員只能選取訂閱的移轉到期日之前的日期。 管理員確認後，使用者移轉將排程在選取的日期開始處理。
...
按一下「移轉所有使用者」按鈕後，系統會提示管理員確認所有合格使用者的移轉。 管理員確認後，使用者移轉將儘快開始處理。
