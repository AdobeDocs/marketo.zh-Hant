...
管理員可選擇未來8到30天的日期。 選取日期時，管理員必須按一下 **儲存** 進行變更。

![](assets/migrating-to-adobe-identity-3.png)

>[!NOTE]
>
>若要在8天前或超過30天後請求日期，或如果您需要在移轉前主控台鎖定後調整日期，請傳送電子郵件至 `marketocares@marketo.com`.
...
移轉狀態頂端會顯示訂閱移轉到期日和延長到期日的按鈕。 有關移轉到期日的詳細資訊，請參閱 [使用者移轉到期部分](#user-migration-expiration).
...
進行移轉的管理員必須按一下 **完整移轉** 按鈕。 系統將會提示使用者 **確認**.
...
按一下 **延長有效期** 按鈕，日期將在一週後更新。 管理員最多可將到期日延長三次。
...
當管理員按一下 **驗證電子郵件** 按鈕，他們會收到電子郵件已傳送的通知。
...
若要略過使用者，管理員可以選取想要的使用者。 「略過移轉」按鈕將變成可點按。 按一下 **略過移轉** 按鈕，頁面將會重新整理，而所選使用者的驗證狀態和移轉狀態將會更新為「已略過」。

![](assets/migrating-to-adobe-identity-16.png)

如果確定需要移轉使用者，管理員可以取消略過先前略過的使用者。

若要取消略過使用者，管理員可以選取想要的使用者。 「未略過移轉」按鈕會變成可點按的狀態。 按一下 **未略過移轉** 按鈕，頁面將會重新整理。  所選使用者的驗證狀態將更新為其目前狀態（「已驗證」或「未驗證」），而使用者的移轉狀態將更新為「未開始」。
...
按一下 **立即移轉** 按鈕時，系統會提示管理員確認所選使用者的移轉。 管理員確認後，使用者移轉將儘快開始處理。
...
按一下 **排程移轉** 按鈕時，系統會提示管理員選取所選使用者的所需移轉日期。 管理員只能選取訂閱的移轉到期日之前的日期。 當管理員確認時，使用者移轉將排程在選取的日期開始處理。
...
按一下 **移轉所有使用者** 按鈕時，系統會提示管理員 **確認** 移轉所有合格使用者。 管理員確認後，使用者移轉將儘快開始處理。
