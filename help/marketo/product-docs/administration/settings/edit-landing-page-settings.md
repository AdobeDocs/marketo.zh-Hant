...
您可以編輯網域名稱和遞補頁面、啟用或停用表單預填、防止登陸頁面誤用等等。 請遵循下列步驟。
...
1. 輸入您的網域和頁面資訊。
   ...
   | 術語 | 定義 |
   |---|---|
   | [!UICONTROL Domain name for landing pages] | 這是您的CNAME。 CNAME是您為登陸頁面提供的網址(URL)的第一部分。 例如，在`https://go.yourCompany.com`中，「go」是CNAME。 您可以有多個，但大部分人只能使用一個。 |
