...
您的行銷人員必須從每個個別例項，邀請您使用相同登入方式加入您的每個不同訂閱。 Marketto無法自動合併您現有的登入。 一旦您啟用Universal ID, **您的Marketo例項將在** 30分鐘內無法使用。 如果您的使用者群較大，可能會比較長。

>[!CAUTION]
>
>如果使用者已啟用「單一ID」或「通用ID」，則其角色和工作區在 **初次設定** 後無法編輯。
