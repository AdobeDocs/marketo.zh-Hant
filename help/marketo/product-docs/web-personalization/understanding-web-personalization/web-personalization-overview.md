...
description: Web個人化概觀 — Marketo檔案 — 產品檔案
title: Web個人化概觀
...
>[!NOTE]
>
>Web Personalization和ContentAI提供所有Marketo支援的語言（英文、法文、德文、日文、葡萄牙文、西班牙文）。

現有客戶可繼續透過「即時個人化」磚存取應用程式，直到續訂為止，屆時他們將使用上述登入。

![](assets/image2016-2-9-8-3a52-3a32.png)

按一下「網頁個人化」動態磚，就會顯示「控制面板」頁面。 按一下ContentAI圖磚，然後直接前往「內容Recommendations」頁面。
...
此 **網頁個人化** 圖磚是您建立個人化網頁行銷活動的存取點。 以帳戶為基礎的網頁行銷與網頁重新目標定位。 您可以在「內容分析」頁面上新增內容。

**帳戶型網頁行銷** 可與Web個人化及指定帳戶功能搭配使用。 「具名帳戶」頁面提供有關現有客戶、參與總計和組織趨勢的圖形資訊，以及帳戶清單。 您也可以在這裡建立新的具名帳戶。

**網站重新目標定位** 可讓您對分段受眾進行再行銷。 您可以設定區段，但不能在篩選中使用具名帳戶。

**contentai** 是提供內容建議的地方。 您可以輕鬆地將內容新增至「內容Recommendations」頁面。

>[!NOTE]
>
>若要存取以帳戶為基礎的網頁行銷或網站重新目標定位，您必須按一下「網頁個人化」方塊，即使您沒有網頁個人化應用程式亦然。

## 組織 {#organizations}

「組織」標籤會顯示指定期間內造訪您網站之組織的所有詳細資料（名稱、位置、活動和時間戳記）。 表格可依時間、位置、網域及透過任意文字搜尋來排序及組織。

![](assets/image2014-11-10-19-3a23-3a18.png)

>[!TIP]
>
>「最活躍」與「最新」 — 表格先由已識別的組織整理，然後由網際網路服務提供者（以ISP圖示表示）。 可依下列條件排序：
>
>* 最活躍：表格中最活躍的組織（根據頁面檢視次數）
>* 最新：表格中最近的組織（預設）
>* 如需有價值的深入分析，請根據最活躍的人進行篩選
...
<table> 
 <tbody> 
  <tr> 
   <td><p><strong>試算表圖示</strong>：圖示位於面板的右上角，會下載CSV格式的組織表格以供離線使用</p><p><strong>設定電子郵件警示</strong>：每次在選取的組織造訪網站時，都會傳送電子郵件給已登入的使用者</p><p><strong>銷售機會</strong>：開啟所選組織的銷售機會表格</p><p><strong>共用</strong>：開啟快顯視窗，透過電子郵件傳送及分享瀏覽過網頁之組織的詳細資訊</p><p><strong>位置圖示</strong>：顯示所選組織的國家/地區和城市</p><p><strong>連結</strong>：Jigsaw、Whois、Linkedin、Google、Twitter會開啟新視窗，其中包含個別網頁的其他資訊，提供所選組織的額外深入分析</p><p><strong>搜尋</strong>：顯示訪客透過搜尋引擎到達時使用的關鍵字</p><p><strong>反向連結</strong>：顯示將流量反向連結至您網站的頁面之URL連結</p><p><strong>訪客</strong>：顯示瀏覽次數、所選組織檢視的頁面數及其電子郵件地址（如果他們已完成提交表單），以及上次瀏覽的時間和日期</p><p><strong>點按資料流</strong>：顯示一個表格，內含網站上的使用者活動和URL路徑，以及使用者造訪每個頁面的時間</p></td> 
...
這會啟用「共用此組織」覆蓋，您可在此傳送共用造訪網站之組織名稱和詳細資料的電子郵件。
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colspan="1" rowspan="1">圖示位於面板的右上方，可下載CSV格式的訪客表格供離線使用</td> 
  </tr> 
  <tr> 
...
「即時」頁面會即時顯示您網站上的訪客，並測量活動發生的時間。

![](assets/image2014-11-10-19-3a49-3a55.png)

「即時」頁面分為6個資訊型表格，提供網站上的訪客詳細資訊：
