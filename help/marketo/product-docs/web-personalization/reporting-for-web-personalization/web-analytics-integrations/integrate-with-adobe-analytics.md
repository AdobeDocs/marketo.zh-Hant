...
# 與Adobe Analytics整合 {#integrate-with-adobe-analytics}
...
1. 前往AA **帳戶中的** 「管理工具」。
1. 選取 **要與整合搭配使用的報表套裝** 。
1. 在「 **編輯設定** 」下，前往「 **轉換** 」並選取「** [轉換變數](http://microsite.omniture.com/t2/help/en_US/reference/#Edit_conversion_variables)」**。\
   選取「 [轉換變數](http://microsite.omniture.com/t2/help/en_US/reference/#Conversion_Variables_eVar) 」編號（我們建議）:
   ...
   1. 
      ...
      1. 將狀態變更為*啟用*
         ...
         1. 將「名稱」變 **更為「** Industry **」**&#x200B;和「Organization」。 （這是報表套裝中的顯示方式。）

         1. 將「過期時間」欄位變更為「 **瀏覽」。**

1. 在「**編輯設定」 **下，前往「**&#x200B;轉換&#x200B;**」並選取「** [成功事件」](http://microsite.omniture.com/t2/help/en_US/reference/#Configure_success_events)**。
...
1. 選取自訂成功事件事件編號（我們建議）:
   ...
   1. 將兩個事件名稱更 **改為RTP促銷活動****和RTP區段**。 此名稱將會出現在報表套裝中。

1. 選擇「類型* *」欄位為「計 **數器」（無子關聯）**

1. 在「 **編輯設定** 」下，前往** [流量](http://microsite.omniture.com/t2/help/en_US/reference/#Traffic_Variable)**，並選取** [流量變數](http://microsite.omniture.com/t2/help/en_US/reference/#Enable_traffic_variable_reports)**。
...
1. 在「 **Account Settings**（帳戶設定）」下，按 **一下「Domain**（網域）」。
1. 在「 **Analytics」下，按一下****「Adobe Analytics**」。
1. 開啟**On **轉換、自訂和流量變數切換。
1. 指派轉換、事件和流量變數 **槽號** ，以符合在AA中建立的槽號
1. 按一 **下儲存**。
...
建立新 [的儀表板](http://microsite.omniture.com/t2/help/en_US/sc/user/t_dashboard_add.html)，稱為 **RTP儀表板**。 此控制面板將協助您瞭解依垂直產業和RTP區段劃分的訪客。

1. 按一 **下控制面板** ，按一下**新增控制面板**

1. 命名儀表板 **RTP儀表板**
1. 選擇控 **制面板大小** 3 x 2、2 x 2
1. 建立小 [報告](http://microsite.omniture.com/t2/help/en_US/sc/user/t_dashboard_add_report.html#task_EC3AFBBAA51C45CEBAF632F841C305B3) ，並將 [內容新增至控制面板](https://docs.marketo.com/Add%2520content%2520to%2520a%2520dashboard)
...
## 在Adobe Analytics中檢視印象和點按（轉換） {#view-impressions-and-clicks-conversions-in-adobe-analytics}
