...
提升電子郵件傳遞率的快速方法之一就是將 **SPF** (寄件者原則架構) 和 **DKIM** (Domain Keys Identified Mail) 納入您的 DNS 設定。 除了您的DNS專案以外，您也會告訴收件者，您已授權Marketo代表您傳送電子郵件。 若未進行此變更，您的電子郵件被標記為垃圾郵件的可能性會較高，因為電子郵件雖然顯示寄件網域為您的網域，但實際卻是由 Marketo 網域的 IP 位址傳送。
