...
**IP熱身程式** — 一段時間後會建立信譽。 有些網際網路服務提供者(ISP)會限制沒有磁碟區記錄的IP位址，因此您必須在最初幾週建立信譽（Marketo可協助您完成相關操作）。
...
**好室友** — 如果分享IP的寄件者遵循最佳傳送實務准則，您將會受益。
**郵寄頻率** — 與專用IP不同，沒有符合共用IP資格的最低郵寄頻率。
**費用** — 從共用IP傳送時一律不需額外付費。

_缺點_

**不好的室友** — 如果分享IP的寄件者採用不良的傳送方式，可能會對您的電子郵件行銷活動造成不良影響。
**控制** — 您對寄件者信譽的控制權要少得多。
**問題解決** — 從共用IP傳送時，通常更難解決問題。

>[!NOTE]
>
>做出決定時，還需要考慮另一個重要因素：傳送數量。 如果您計畫每月傳送少於100,000封電子郵件，或每月少於兩封郵件，則可能無法受益於專用IP。 這類傳送的數字會被視為很低，很難讓您的專用IP保持「溫暖」，以讓主要ISP認為安全。 如果您信件傳送頻率不夠高，ISP會看到郵件突然出現「激增」的活動，最終可能會將其封鎖為可疑垃圾郵件。

如果您有任何問題或想設定固定IP，請聯絡您的Marketo客戶代表。
