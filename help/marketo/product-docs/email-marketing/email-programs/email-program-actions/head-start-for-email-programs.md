...
1. 在「排程」圖磚中，排程您的電子郵件，然後選取 **開始時間** 方塊。
   ...
   >[!CAUTION]
   >
   >您的對象中，在方案鎖定後取消訂閱的任何人仍會收到電子郵件。 建議您調整取消訂閱通知，以反映取消訂閱可能需要1-2個工作日才能處理。
...
1. 按一下 **核准計畫**.
   ...
   * **正在等候執行：** 在方案核准後。
   * **處理已開始，正在等候執行：** 處理進行中。
   * **處理完成，正在等候執行：** 處理完成，電子郵件現在正等待排定的啟動時間。
   * **已完成：** 程式已完成。

   >[!TIP]
   >
   >想要在程式鎖定後但在電子郵件傳送前取消？ 沒問題！ 只需按一下 **中止程式** 在「核准」圖磚的右下方。
...
我們現有的「開門即用」功能要求程式至少提前12小時排程。 這對收件者時區有何意義？ 回想一下，當收件者時區作用中時，我們會在最早時區的午夜開始執行電子郵件程式(UTC +14:00)。 因此，若要啟用 **兩者** 開始時間和收件者時區，需要排程程程式 **最早時區之前至少12小時(UTC +14:00**.)

這代表如果您位於美洲/洛杉磯，且想要同時啟用開頭和收件者時區，則需排程程方案 **34小時** 事前準備。 我們如何取得這個號碼？

![](assets/image2017-12-5-13-3a11-3a46.png)

[瞭解更多](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/scheduling-with-recipient-time-zone/schedule-email-programs-with-recipient-time-zone.md) 關於如何使用收件者時區排程電子郵件程式。
