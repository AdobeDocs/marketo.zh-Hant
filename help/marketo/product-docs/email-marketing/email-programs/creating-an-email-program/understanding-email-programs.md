---
unique-page-id: 2953572
description: 瞭解電子郵件程式和控制面板。 在一個位置傳送電子郵件、執行A/B測試和檢視控制面板。
title: 了解電子郵件方案
exl-id: 3ec1a4f0-90fb-4a4b-94bf-ce5ee7d8bdd4
feature: Email Programs
source-git-commit: 7eb2f49718ea02be4a394a142c3a0ff05eeff796
workflow-type: tm+mt
source-wordcount: '327'
ht-degree: 3%

---

# 了解電子郵件方案 {#understanding-email-programs}

傳送電子郵件、建立A/B測試，以及追蹤您的結果，全都集中在一處。

## 控制面板 {#control-panel}

四個方塊讓您保持焦點。 **[!UICONTROL Audience]**&#x200B;定義「誰」，**[!UICONTROL Email]**&#x200B;提供「什麼」，**[!UICONTROL Schedule]**&#x200B;定義「何時」，**[!UICONTROL Approval]**&#x200B;使其準備就緒。

![](assets/emailprogram.png)

## A/B 測試 {#a-b-testing}

A/B測試可讓您證明哪封電子郵件最好。

瞭解下列A/B測試：

* [「主旨列」A/B測試](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/email-test-a-b-test/use-subject-line-a-b-testing.md)
* [「整個電子郵件」A/B測試](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/email-test-a-b-test/use-whole-email-a-b-testing.md)
* [「寄件者地址」A/B測試](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/email-test-a-b-test/use-from-address-a-b-testing.md)
* [「日期/時間」 A/B測試](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/email-test-a-b-test/use-date-time-a-b-testing.md)

![](assets/abtesthighlight.png)

## 儀表板 {#dashboards}

>[!CAUTION]
>
>為獲得準確的報告，請避免&#x200B;_重複使用電子郵件程式的_&#x200B;電子郵件，方法是在智慧行銷活動中參考該電子郵件，或將資產從已啟動的電子郵件程式移至新的電子郵件程式。 這麼做會彙總該電子郵件附加之每個報告控制面板中的所有資料。 如果您需要重複使用電子郵件，請改為複製[它](/help/marketo/product-docs/core-marketo-concepts/programs/working-with-programs/clone-an-asset-in-a-program.md){target="_blank"}，因為這樣會複製電子郵件，但會建立具有新電子郵件識別碼的新電子郵件。

享受您勞動的成果。 控制面板會一眼顯示電子郵件程式和A/B測試效能。

![](assets/image2015-4-27-11-3a38-3a41.png)

![](assets/image2015-4-27-11-3a38-3a27.png)

>[!TIP]
>
>有些使用者偏好使用電子郵件程式，而非透過Smart Campaign傳送電子郵件，因為此程式有詳細的報表控制面板。

## 巢狀 {#nesting}

電子郵件程式可與其他程式搭配使用。

![](assets/image2015-4-27-11-3a49-3a22.png)

例如，使用電子郵件程式來傳送網路研討會邀請。

![](assets/image2015-4-27-12-3a20-3a40.png)

在預設程式中使用電子郵件程式，讓事情維持井然有序。

>[!TIP]
>
>若要將現有的電子郵件程式複製到另一個程式，您必須複製該程式。

>[!NOTE]
>
>電子郵件程式無法巢狀化至[參與程式](/help/marketo/getting-started/quick-wins/drip-drip-nurture.md)。 它們也無法重新傳送，一次只能執行一項測試。

## 排程設定 {#schedule-settings}

現在您已瞭解電子郵件計畫，開始使用下列其中一個相關文章！

>[!MORELIKETHIS]
>
>* [建立電子郵件程式](/help/marketo/product-docs/email-marketing/email-programs/creating-an-email-program/create-an-email-program.md)
>
>* [使用智慧清單定義對象](/help/marketo/product-docs/email-marketing/email-programs/managing-people-in-email-programs/define-an-audience-with-a-smart-list.md)
>* [為電子郵件程式建立電子郵件](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/create-an-email-for-an-email-program.md)
>* [排程您的電子郵件程式](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/schedule-your-email-program.md)
