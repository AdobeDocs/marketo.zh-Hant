...
1. 在啟動之前執行[驗證程式](/help/marketo/product-docs/coworker-for-marketo/skills/validate-programs.md)代理程式。

## 使用案例 {#use-cases}

**網路研討會註冊計畫**：行銷活動經理輸入「為我們8月份的產品示範建立網路研討會註冊計畫」。 傳送邀請電子郵件、前一天提醒，以及後續的錄製連結。」 Marketo Engage的同事會建立包含三個智慧行銷活動（邀請、提醒、跟進）、每個活動的預留位置電子郵件，以及根據事件日期排程的方案。
...
* 不需要預先使用範本或表單。 Marketo Engage的同事建立結構，之後您可以連線資產（您仍需負責撰寫電子郵件復本及設定任何登入頁面）。
* Marketo Engage的同事無法自動存取您現有的對象清單。 程式建立後，您必須將智慧清單篩選器連線到實際區段。
