...
description: 瞭解如何為Dynamics 365設定具有伺服器對伺服器連線的Marketo解決方案。 在Azure AD中建立使用者端應用程式，並授與同步處理使用者的同意。
...
>[!CAUTION]
>
>請注意「使用者端密碼」值（見以下熒幕擷圖），因為它只會顯示一次，無法再次擷取。
...
1. 請依照下列連結中的步驟操作，[在Microsoft](https://docs.microsoft.com/en-us/powerapps/developer/common-data-service/use-single-tenant-server-server-authentication#application-user-creation){target="_blank"}中設定應用程式使用者。
   ...
   >[!IMPORTANT]
   >
   >* 將許可權授與應用程式使用者時，請將其指派給「Marketo同步使用者角色」。
...
其他參考[可以在這裡找到](https://docs.microsoft.com/en-us/azure/active-directory/reports-monitoring/concept-all-sign-ins#:~:text=Interactive%20user%20sign%2Dins%20are,as%20the%20Microsoft%20Authenticator%20app.&text=This%20report%20also%20includes%20federated,are%20federated%20to%20Azure%20AD.){target="_blank"}。

## 設定Marketo解決方案 {#configure-marketo-solution}

最後一個步驟：通知Marketo解決方案有關建立的新使用者。

1. 返回[!UICONTROL Advanced Settings]區段並按一下[!UICONTROL Settings]旁的![](assets/image2015-5-13-15-3a49-3a19.png)圖示，然後選取&#x200B;**[!UICONTROL Marketo Config]**。
...
1. 按一下[!UICONTROL Settings]旁的![](assets/image2015-5-13-15-3a49-3a19-1.png)圖示，然後選取&#x200B;**[!UICONTROL Solutions]**。
