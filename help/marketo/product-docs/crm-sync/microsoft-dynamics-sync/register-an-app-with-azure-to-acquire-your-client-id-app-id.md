...
description: 瞭解如何向Azure註冊應用程式，以取得您的使用者端ID和應用程式ID以進行Dynamics同步。 使用Azure Active Directory和應用程式註冊進行驗證。
...
1. [使用具有管理員許可權的帳戶登入](https://login.microsoftonline.com/){target="_blank"} Microsoft Azure管理入口網站。 您也可以展開左側導覽窗格中的&#x200B;**[!UICONTROL Admin]**&#x200B;專案並選取&#x200B;**[!UICONTROL Azure AD]**，以透過Office 365系統管理中心存取Microsoft Azure入口網站。
   ...
   >[!NOTE]
   >
   >如果您沒有Azure帳戶，您可以[註冊](https://azure.microsoft.com/en-us/free/){target="_blank"}帳戶。 如需詳細資訊，請參閱Microsoft檔案或聯絡Microsoft代表。 建立Azure帳戶後，您可以使用下列程式註冊一或多個應用程式。
   >
   >
