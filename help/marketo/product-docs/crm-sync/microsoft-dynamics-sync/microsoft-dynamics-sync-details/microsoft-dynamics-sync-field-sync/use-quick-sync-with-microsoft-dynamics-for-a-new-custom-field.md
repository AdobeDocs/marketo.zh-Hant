...
一般而言，快速同步是用來新增欄位及重新整理值。 但是，在某些情況下，您可能會想要同步現有欄位。 您可以根據更新或建立的日期範圍來限制欄位同步。 如需詳細資訊，請參閱下方的[進階同步選項](#Advanced_Sync_Options)。
...
如果您想要同步現有欄位，但只針對有限的資料集進行同步處理，該怎麼做？
...
在[!DNL Dynamics]與Marketo欄位不同步的罕見情況下，可以快速重新同步它們。
