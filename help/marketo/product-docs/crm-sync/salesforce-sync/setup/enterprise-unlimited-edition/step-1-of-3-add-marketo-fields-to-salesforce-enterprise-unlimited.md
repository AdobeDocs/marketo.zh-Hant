...
description: 第1步（共3步） — 將Marketo欄位新增至Salesforce（企業版/無限製版） — Marketo檔案 — 產品檔案
title: 第1步（共3步） — 將Marketo欄位新增至Salesforce（企業版/無限製版）
...
# 第1步（共3步）:將Marketo欄位新增至Salesforce（企業版/無限製版） {#step-of-add-marketo-fields-to-salesforce-enterprise-unlimited}

>[!PREREQUISITES]
>
>您必須有Salesforce API的存取權，才能在Marketo和Salesforce之間同步。
...
對三個自訂欄位中的每個欄位執行下列步驟以新增它們。 從分數開始。

1. 登入Salesforce，然後按一下 **設定**.

   ![](assets/image2016-5-23-13-3a15-3a21.png)

1. 在左側的「建置」功能表中，按一下 **自訂** 選取 **銷售機會**. 按一下 **欄位**.

   ![](assets/image2016-5-23-13-3a20-3a5.png)

1. 按一下 **新增** 在頁面底部的「自訂欄位與關係」區段中。

   ![](assets/image2016-5-26-14-3a41-3a40.png)

1. 選擇適當的欄位類型(對於分數 — 數字；採購方案 — 文本；收購日期 — 日期/時間)。
...
1. 指定存取設定，然後按一下 **下一個**:
   ...
   * 清除 **唯讀** 同步用戶配置檔案的複選框：
      ...
      * 如果您的使用者的設定檔為 _系統管理員_ 作為同步用戶，請清除 **唯讀** 「系統管理員」配置檔案的複選框（如下所示）
      * 如果您已建立 _自訂設定檔_ 對於同步用戶，請清除 **唯讀** 該自訂設定檔的核取方塊
...
1. 按一下 **儲存並新增** 返回並建立其他兩個自訂欄位。 按一下 **儲存** 三件事都沒了。

   ![](assets/image2016-5-23-15-3a8-3a43.png)

1. 在左側的「建置」功能表中，按一下 **自訂** 並選擇「聯繫人」。 按一下欄位。
...
1. 在導覽搜尋中輸入「欄位」，而不按Enter鍵。 欄位會顯示在不同的物件下；按一下 **欄位** 在Lead下。

   ![](assets/image2016-5-26-16-3a36-3a32.png)

1. 前往「銷售機會自訂欄位與關係」區段，然後按一下 **映射銷售機會欄位**.
