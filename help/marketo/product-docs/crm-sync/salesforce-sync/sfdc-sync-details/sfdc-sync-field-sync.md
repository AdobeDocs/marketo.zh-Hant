...
Marketo中有一個名為「SFDC型別」的欄位。 它有三個可能的值：銷售機會、聯絡人或它是空的。 如果空白，表示此Marketo銷售機會不存在於SFDC中。

## 如何判斷在SFDC中是否刪除潛在客戶或連絡人？ {#how-do-you-determine-if-a-lead-or-contact-is-deleted-in-sfdc}

Marketo中有一個名為SFDC isDeleted的欄位。 如果值為true，則潛在客戶已在SFDC中刪除。

## 如何確定我在SFDC中新增的新欄位也會新增至Marketo？ {#how-do-i-make-sure-a-new-field-i-add-in-sfdc-also-gets-added-to-marketo}

>[!TIP]
>
>如果您想要在兩個系統中都使用欄位，請先在SFDC中建立該欄位，它會自動同步至Marketo。

如果您在SFDC中新增欄位，且同步使用者有檢視該欄位的許可權，該欄位會自動新增至Marketo。
...
## 如果我變更SFDC中的API名稱，該怎麼辦？ {#what-if-i-change-an-api-name-in-sfdc}
...
公式欄位會同步，但公式中參照的更新要等到更新後才同步。 [系統修改印章](https://help.salesforce.com/apex/HTViewSolution?id=000193203&amp;language=en_US).

## 當我從Salesforce中刪除先前與Marketo同步的欄位時會發生什麼情況？ {#what-happens-when-i-delete-a-field-from-salesforce-that-was-previously-syncing-with-marketo}
