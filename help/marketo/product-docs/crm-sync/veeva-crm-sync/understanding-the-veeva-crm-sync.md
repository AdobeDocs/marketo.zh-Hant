---
description: 瞭解Veeva CRM同步如何在Marketo Engage和Veeva之間運作。 執行同步並檢視同步的內容，包括個人帳戶和自訂物件。
...
執行Adobe Marketo Engage與[!DNL Veeva] CRM之間的同步只需要幾個步驟。
...
[!DNL Veeva]與Marketo Engage之間的同步只對個人帳戶物件上的連絡人欄位是雙向的。 在這些情況下，只要您在[!DNL Veeva]或Marketo Engage中進行變更，您的更新就會反映在兩個系統中。 所有其他同步僅從[!DNL Veeva]到Marketo Engage。 按一下下列連結，以取得每個項目的詳細資料。
...
* 您在Marketo Engage中為 [!DNL Veeva]](/help/marketo/product-docs/crm-sync/salesforce-sync/setup/enterprise-unlimited-edition/step-2-of-3-create-a-salesforce-user-for-marketo-enterprise-unlimited.md){target="_blank"}輸入的[認證可用來同步資料。 只有這些認證可存取的資料才會包括在內。
