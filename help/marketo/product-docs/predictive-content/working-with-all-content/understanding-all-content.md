...
* **檢視**：對探索到的網頁內容的總點按次數（包括所有來源）
* **轉換**：在同次造訪中檢視內容片段並完成表單的訪客數量
...
選取一或多個類別以篩選內容。 然後，使用標準或自訂設定來設定內容分析的日期或日期範圍。

1. 於 **所有內容** 頁面上，按一下篩選器圖示，然後在 **類別**，選取一或多個內容類別。
...
1. 若要顯示特定日期或日期範圍的資料，請按一下日曆圖示，然後從左側面板選取。 若是自訂日期範圍，請按一下行事曆上日期範圍的第一天和最後一天（如所示）。 按一下 **套用**.
