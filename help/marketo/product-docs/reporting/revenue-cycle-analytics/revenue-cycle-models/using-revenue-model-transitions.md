...
1. 轉換可控制潛在客戶在階段之間的移動方式。 從右側拖曳您選擇的觸發器（或篩選器），然後隨處釋放到畫布上。 在此範例中，我們將選取「填寫表單」觸發器。

   >[!TIP]
   >
   >由於收入模型程式正在為您設定報表，因此建議轉變一律包含觸發器。 如此一來，您的報表就會反映模型/階段流程的真實速度。 篩選器可以與觸發器一起新增，以取得其他限制。
