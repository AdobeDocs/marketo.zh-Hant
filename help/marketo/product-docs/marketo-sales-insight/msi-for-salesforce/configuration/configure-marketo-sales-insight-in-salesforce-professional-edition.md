...
以下是設定Salesforce Professional Edition中Marketo Sales Insight的必要步驟。 讓我們開始吧。
...
1. 開啟新的瀏覽器視窗，以從您的Marketo帳戶取得Marketo Sales Insight認證。
1. 前往管理區域並選取 **銷售分析**.
...
1. 輸入遠端網站名稱（類似「MarketoRestAPI」）。 輸入遠端網站URL，這是您從Marketo的Rest API設定面板取得的API URL。 按一下 **儲存**. 您現在已建立Rest API的遠端站台設定。

## 設定Marketo Sales Insight {#set-up-marketo-sales-insight}

1. 登入您的Marketo執行個體並按一下 **管理員**.
...
1. 輸入 **遠端站台名稱** 和 **遠端網站URL**，然後按一下 **儲存**.

   ![](assets/remote-site-1.png)

   >[!NOTE]
   >
   >您選擇您的 **遠端站台名稱** （此處使用MarketoAPI）。 此 **遠端網站URL** 您可在步驟4中「編輯API設定」對話方塊的「Marketo主機」欄位中找到。
...
1. 按一下 **欄位** 左側。 然後搜尋並拖曳 **參與** 標籤到 **Marketo Sales Insight** 版面。
...
1. 重複此程式以新增Visualforce頁面區段和Sales Insight欄位 **連絡人**， **帳戶** 和 **機會**.
1. 重複步驟5-7以新增Contact、Account和Opportunity的Visualforce頁面區段。 然後，重複步驟8至10，新增Sales Insight欄位 **連絡人**. 進行任何變更後，請務必儲存。
...
>[!MORELIKETHIS]
>
>* [優先順序、緊急程度、相對分數和最佳比對](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/stars-and-flames/priority-urgency-relative-score-and-best-bets.md){target="_blank"}
