...
>[!PREREQUISITES]
>
>* 在開始設定MSI之前，為您的Marketo實例啟用了「MSI非本地」功能(如果尚未啟用，並且您已購買了該功能，請聯繫[Marketo支援](https://nation.marketo.com/t5/support/ct-p/Support) — 如果您尚未購買此功能，請聯繫您的客戶成功經理)。
>* 設定了[MSI包](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/installation/install-marketo-sales-insight-package-in-salesforce-appexchange.md)的Salesforce帳戶。
>* Marketo REST API [已成功設定](https://developers.marketo.com/rest-api/)。 公開的CRUD API將是執行非原生同步的基礎。
>* 請閱讀[此部落格文章](https://developers.marketo.com/blog/create-and-associate-leads-companies-and-opportunities-with-the-marketo-rest-api/)以了解物件和關係。
...
1. 將Salesforce Sales使用者同步至Marketo。
   ...
   Salesforce Sales User是在Salesforce中擁有Lead/Contacts的外部用戶。 需要為Salesforce Sales用戶更新Marketo銷售人員。 *externalSalesPersonId*&#x200B;欄位被授權用於Sales Person的更新。
...
用於同步銷售人員的API檔案：[https://developers.marketo.com/rest-api/endpoint-reference/lead-database-endpoint-reference/#!/Sales_Persons/syncSalesPersonsUsingPOST](https://developers.marketo.com/rest-api/endpoint-reference/lead-database-endpoint-reference/#!/Sales_Persons/syncSalesPersonsUsingPOST)

1. 將Salesforce帳戶同步至Marketo。

   需要更新Marketo公司的Salesforce帳戶。 _externalCompanyId_&#x200B;和&#x200B;_externalSalesPersonId_&#x200B;欄位是公司更新的強制欄位。
...
1. 將Salesforce銷售機會/聯繫人同步到Marketo。
   ...
   您需要為Salesforce銷售機會/聯繫人更新Marketo銷售機會。 _externalPersonId_、_externalSalesPersonId_&#x200B;和&#x200B;_externalCompanyId_&#x200B;欄位被授權用於Lead的更新。
...
1. 將Salesforce Opportunity同步至Marketo。
   ...
   您需要為Salesforce Opportunity添加Marketo Opportunity。 _externalOpportunityId_、_externalCompanyId_&#x200B;和&#x200B;_externalSalesPersonId_&#x200B;欄位被授權用於Opportunity的更新。
...
1. 將Salesforce連絡人角色同步至Marketo。
   ...
   然後，可以通過Marketo Opportunity角色同步Salesforce Opportunity的Salesforce聯繫人角色。 「機會角色」記錄授權&#x200B;_externalOpportunityId_、_角色_&#x200B;和&#x200B;_leadId_&#x200B;欄位。
...
1. 將上一個有趣的時刻/MSI計分欄位同步到SFDC。
   ...
   Marketo Lead的「最後有趣的時刻/分數」欄位需要使用REST API Lead端點定期同步至Salesforce。 使用&#x200B;_externalPersonId_&#x200B;作為filterType，並傳入Salesforce Lead GUID作為filterValue，查詢此端點中的Marketo Lead。
