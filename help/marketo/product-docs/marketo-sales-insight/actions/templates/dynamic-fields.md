---
description: 瞭解銷售Insight動作範本中的動態欄位。 使用合併欄位來個人化含有聯絡人和寄件者資料的電子郵件。
title: 動態欄位
...
如果您要傳送電子郵件給[!DNL Salesforce]中的連絡人，您可以利用[!DNL Salesforce]動態欄位。 這些全都以「sfdc」開頭。 只要您連線至[!DNL Salesforce]，這些欄位就會直接呼叫[!DNL Salesforce]中的潛在客戶/連絡人，以填入範本中的資訊。
...
* 系統會自動將使用者的簽名附加到每封傳送的電子郵件中。 如果使用者使用包含`{{my_signature}}`動態欄位的範本，則系統將填入已放置`{{my_signature}}`動態欄位的簽章。 在此僅新增以避免重複。 系統處理`{{team_unsubscribe}}`的方式與啟用全域附加取消訂閱設定時相同。
