...
**最新動態：** 使用者沒有執行「最近活動」記錄交易所需的「銷售分析動作」自訂欄位的寫入許可權。 Team可能已經安裝套件，但尚未為使用者啟用正確的欄位。\
...
**疑難排解步驟：** 手動更新缺少欄位的值。 然後，您可以從「銷售分析動作」重試訊息。
...
**類別：** 存取/驗證\
**訊息：** 由於貴組織有安全性原則，因此不允許您要求的操作。 請聯絡您的管理員。**最新動態：** 已設定某種安全性限制 — 請參閱https://developer.salesforce.com/forums/?id=「記錄ID」\
