...
**發生狀況：**&#x200B;存取或重新整理Token已過期。 Token會根據 [!DNL Salesforce]](https://salesforce.stackexchange.com/questions/10759/invalid-grant-expired-access-refresh-token-error-when-authenticating-access-via)中的[工作階段設定到期。
**疑難排解步驟：**&#x200B;您需要重新驗證。 中斷[!DNL Salesforce]連線並重新連線。

**錯誤：** FAILED_WRITE
**類別：**間斷
**訊息：**已到達檔案結尾
**發生狀況：** [!DNL Salesforce]的效能問題，可能是因為客戶端的觸發程式未達最佳狀態。
...
**發生狀況：**&#x200B;我們正在嘗試存取您的[!DNL Salesforce]，但您已經設定IP限制，因此我們無法存取。
...
**類別：**存取/驗證
**訊息：**由於貴組織有安全性原則，因此不允許您要求的作業。 請聯絡您的管理員。
...
**類別：**其他
**訊息：**發生未知的例外狀況
**發生的情況：[!DNL Salesforce]中發生**個未處理的例外狀況。
