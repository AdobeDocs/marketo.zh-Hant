...
**類別：** 存取/驗證\
**訊息：** 無法建立/更新欄位：ToutApp__Tout_Last_Respled__c。請檢查此欄位的安全設定。
**發生的狀況：** 用戶沒有執行「最近」活動記錄事務所需的「輸出」自定義欄位的寫權限。 團隊可能已安裝軟體包，但未為用戶啟用正確欄位。\
...
**疑難排解步驟：** 手動更新缺少欄位的值。 然後，您可以從ToutApp重試該郵件。
...
**類別：** 存取/驗證\
**訊息：** 由於貴組織中的安全策略，不允許您請求的操作。 請聯絡您的管理員。<br/>
