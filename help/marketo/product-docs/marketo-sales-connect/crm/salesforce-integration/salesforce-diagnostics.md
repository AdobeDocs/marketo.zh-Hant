...
**疑難排解步** 驟：請向我們的支援團隊報告此 [類情況](http://nation.marketo.com/community/support_solutions)。
...
**理Token發生什麼：存** 取或重新整理Token已過期。預付碼會根據Salesforce](http://salesforce.stackexchange.com/questions/10759/invalid-grant-expired-access-refresh-token-error-when-authenticating-access-via)中的[作業設定過期。
...
**類別：** 存取／驗證** ** 
**Message:** 對對象ID的存取權限不足* * ** * 
**What ising:** Most Recent Activity logging cant not edit the specific record asse the user not have writ-to access.\
...
**類別：存** 取／驗證\
**消息：** 無法建立／更新欄位：ToutApp_Tout_Last_Resloded__c。請檢查此欄位的安全設定。\
**發生的情況：** 使用者沒有執行「最近活動」記錄交易所需之「輸出」自訂欄位的寫入存取權。團隊可能已安裝軟體包，但未為用戶啟用正確的欄位。\
...
**發生什麼：** 我們嘗試從Salesforce查詢使用者無權存取的物件類型。這很可能與使用者沒有權限存取銷售機會物件有關。\
...
**類別：存** 取／驗證\
**訊息：** 由於貴組織的安全性原則，您請求的作業無法進行。聯絡您的管理員。\
**發生什麼：已** 經設定了某種安全限制——請參閱 `https://developer.salesforce.com/forums/?id="record` ID」\
