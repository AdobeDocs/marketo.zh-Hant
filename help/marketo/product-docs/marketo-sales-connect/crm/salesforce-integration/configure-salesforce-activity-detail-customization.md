---
description: 瞭解如何設定Sales Connect的Salesforce活動詳細資料自訂。 自訂活動在Salesforce中的顯示方式。
...
>[!PREREQUISITES]
>
>* [!DNL Salesforce]與[!DNL Marketo Sales Connect] [必須連線](/help/marketo/product-docs/marketo-sales-connect/crm/salesforce-integration/connect-your-sales-connect-account-to-salesforce.md)
...
活動詳細資料自訂可讓管理員設定當[!DNL Sales Connect]活動/提醒任務同步到Salesforce時，將記錄到Salesforce任務 — 主題欄位的資訊。

>[!NOTE]
>
>* 如果您在活動詳細資料自訂中使用[!DNL Sales Connect]動態欄位，提醒任務[!DNL Salesforce]中主題欄位的更新將會反映在對應`{{activity_subject}}`任務的主題欄位中。
...
* 管理員可以使用唯一識別碼（例如「Mkto_sales」）標籤主旨欄位，以便輕鬆識別來自Sales Connect的活動，並與其他電子郵件活動、通話活動和任務區分開來。
* 減少自訂活動欄位的需求。 Salesforce會強制限制自訂活動欄位的數量，以限制可用於報表中的資料。 透過使用活動動態欄位將關鍵資料新增到主旨行，您可以減少在Salesforce執行個體中需要建立的自訂活動欄位數量。
* 活動和任務的主題欄位將遵循Sales Connect管理員定義的一致模式。
...
>[!NOTE]
>
>如果沒有值可填入特定活動/任務的動態欄位，則當更新[!DNL Salesforce]任務 — 主題欄位時，不會填入該動態欄位的任何資料。
...
1. 在活動詳細資料自訂編輯器中，新增您想要的任何任意任意文字。 您新增的文字為非動態文字，且同步至[!DNL Salesforce]的所有任務的主旨欄位將維持不變。
   ...
   >[!TIP]
   >
   >雖然不需要，但將新增的文字繞排成直括弧可以讓某些人在資料填入Salesforce的主題欄位時，更容易分辨這些資料。 範例：`[Sales Connect] - {{Activity_type}}`
...
>* [提醒工作與 [!DNL Salesforce]](/help/marketo/product-docs/marketo-sales-connect/tasks/reminder-task-sync-with-salesforce.md)同步
>* [[!DNL Sales Connect] CRM的自訂](/help/marketo/product-docs/marketo-sales-connect/crm/salesforce-customization/sales-connect-customizations-for-crm.md)
