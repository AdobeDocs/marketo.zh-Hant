---
unique-page-id: 14746161
description: 瞭解Sales Connect中的取消訂閱。 使用Salesforce管理選擇退出、取消訂閱連結及同步取消訂閱。
title: 取消訂閱概觀
exl-id: 3212d5be-07ea-47bd-874e-25d02513c23b
feature: Marketo Sales Connect
source-git-commit: 15427eacd2fc42a02f6a4c59d9102bacba02e57b
workflow-type: tm+mt
source-wordcount: '99'
ht-degree: 4%

---

# 取消訂閱概觀 {#unsubscribe-overview}

遵守電子郵件隱私權法規對組織而言越來越重要。 為了協助解決此問題，我們對取消訂閱體驗進行了一些增強功能。

* 從[!DNL Sales Connect]/ToutApp和[!DNL Salesforce]傳送的所有電子郵件都會放置取消訂閱連結（這不適用於從[!DNL Outlook]或Gmail傳送的自訂電子郵件）
* 管理員可以編輯整個團隊的取消訂閱訊息
* 取消訂閱資訊儲存在PDV中
* 取消訂閱可以手動完成：點選連結、[!DNL Salesforce]同步和退回
* 新的取消訂閱連結登陸頁面
