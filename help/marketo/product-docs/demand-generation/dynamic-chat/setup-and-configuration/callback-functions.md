...
您可以使用Dynamic Chat Widget回呼函式，將交談事件傳送至任何協力廠商平台。

## 快速入門 {#getting-started}

此事件代表Dynamic Chat Widget已可供使用，並在網頁中載入與Dynamic Chat相關的所有指令碼時引發。
...
如果您想要將任何這些事件傳送至Adobe Analytics或Google Analytics等分析平台，則必須在這些Dynamic Chat事件中新增其各自的追蹤呼叫。 它看起來會類似於下面的範例。
