...
>[!PREREQUISITES]
>
>確認您的代理程式已獲得適當的[許可權](/help/marketo/product-docs/demand-generation/dynamic-chat/setup-and-configuration/permissions.md){target="_blank"}。
...
>[!IMPORTANT]
>
>* 代理程式收件匣&#x200B;**中的[可用性切換](/help/marketo/product-docs/demand-generation/dynamic-chat/live-chat/agent-inbox.md#availability-toggle){target="_blank"}將覆寫**&#x200B;在&#x200B;_即時聊天可用性_&#x200B;索引標籤中輸入的內容。 因此，如果代理程式排程為1p-5p可用，但需要在3p進行快速中斷，則他們不需要變更其代理程式設定。 可用性切換狀態將一直保留，直到手動變更為止，直到達到代理程式可用性的下一個時間區塊，或直到代理程式指定時區的午夜（如需詳細資訊，請參閱下一個專案符號）。
>
