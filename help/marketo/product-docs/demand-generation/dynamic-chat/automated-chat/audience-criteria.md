...
優先順序決定潛在客戶符合多個對話方塊資格時，會收到哪個對話方塊。 它是您首次[建立對話方塊](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/create-a-dialogue.md){target="_blank"}時建立的。 您可以開啟現有對話方塊並前往「對象條件」標籤中的&#x200B;**[!UICONTROL Dialogue Details]**，以變更其優先順序。

![](assets/audience-criteria-1.png)

## 事件 {#events}

事件可讓您根據訪客的捲動次數或訪客在您的頁面或網站上的時間長度來鎖定訪客。 在以下範例中，設定會鎖定在特定頁面上停留超過20秒的訪客。
...
有&#x200B;_多個_&#x200B;屬性組合可供選擇。 在以下範例中，設定將目標鎖定在加州所有在員工超過50人的公司中工作的已知人員。
...
下列會鎖定資料庫中尚未存取的人員。 在此範例中，設定會鎖定位於New York區域的所有匿名人員。
...
1. 抓取&#x200B;**[!UICONTROL Inferred State]**&#x200B;屬性並將其拖曳到顯示&#x200B;_將屬性拖放到此處_&#x200B;的位置。
   ...
   >[!NOTE]
   >
   >當有人造訪您的網站時，[Munchkin](/help/marketo/product-docs/administration/additional-integrations/add-munchkin-tracking-code-to-your-website.md){target="_blank"}會對其進行Cookie並將其放入系統中。 會在特殊資料庫中查詢IP位址，以推斷位置和其他資訊。
