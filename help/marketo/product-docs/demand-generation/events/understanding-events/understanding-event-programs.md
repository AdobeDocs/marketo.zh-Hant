---
unique-page-id: 2949841
description: 瞭解事件計畫 — Marketo文檔 — 產品文檔
title: 瞭解事件程式
exl-id: 7d62accd-ab2f-46f7-998e-9145c4c11cbc
source-git-commit: 66baa78d3e32140526d8d66beba493e02937a2c2
workflow-type: tm+mt
source-wordcount: '92'
ht-degree: 0%

---

# 瞭解事件程式 {#understanding-event-programs}

>[!AVAILABILITY]
>
>並非所有客戶都購買了此功能。 請與銷售代表聯繫以瞭解詳細資訊。

## 概述 {#overview}

事件程式是一種可用於自動執行離線或聯機事件的程式。 將活動計畫視為在特定日期和時間有與會者的任何活動。

>[!NOTE]
>
>**示例**
>
>* 網路研討會
>* 締約方
>* 貿易展
>* 會議


事件可以自動連接到 [網路研討會提供者](/help/marketo/product-docs/demand-generation/events/understanding-events/event-partners.md) 與 [iPad簽入應用](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/event-check-in/check-people-into-your-event-from-your-tablet.md)。

>[!MORELIKETHIS]
>
>* [建立新事件程式](/help/marketo/product-docs/demand-generation/events/understanding-events/create-a-new-event-program.md)
>
>* [LaunchPoint活動合作夥伴](/help/marketo/product-docs/demand-generation/events/understanding-events/event-partners.md)

