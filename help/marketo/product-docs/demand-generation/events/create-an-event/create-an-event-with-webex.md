...
首先在Webex事件中心建立Webex事件。 Marketo只使用特定設定和欄位來整合，我們稍後將介紹。 要為Webex配置的其他欄位將在 [《 Webex事件中心使用手冊》](https://www.cisco.com/c/dam/en/us/td/docs/collaboration/meeting_center/wbs298/wx_ec_host_ug.pdf)。
...
這些設定僅位於Webex中。 它們在Marketo不被使用或可查看，但它們可能對您的網路研討會非常重要，因此請仔細檢查它們！

## 事件說明和選項  {#event-description-options}

以下選項在Marketo使用或可查看。 其他欄位僅位於Webex中。

* **說明**  — 輸入說明。 這在Marketo是可以看到的，但是不能修改。
* **事後調查** -Marketo目前無法獲取Webex事件後調查的資訊。
...
* **最大註冊者數**  — 當前 **不** 支援Marketo-Webex整合。  使用Marketo的「待批准」進展狀態，可手動審批註冊人。
...
1. 下 **事件**，選擇新建立的Webex活動。 然後，選擇一個可選的備份頁，然後按一下 **保存**。
