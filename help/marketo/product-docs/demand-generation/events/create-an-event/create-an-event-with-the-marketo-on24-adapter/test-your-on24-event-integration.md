...
## 執行第一個行銷活動之前建議的測試順序 {#recommended-test-sequence-before-running-your-first-campaign}

1. 填寫事件登錄檔，並使用有效的電子郵件地址進行測試。
1. 確認測試名稱顯示帶有 **已註冊** Marketo事件之「成員資格」格線中的狀態。
1. 確認測試名稱也會顯示為 **已註冊** 在ON24中。
...
* 在出席者資料在Marketo和ON24之間同步後，Marketo會將會籍狀態更新為已出席、已出席隨選或未顯示。 在事件的 **摘要** 索引標籤中，事件狀態會更新為 **事件完成**.
