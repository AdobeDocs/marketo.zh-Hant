---
unique-page-id: 10096712
description: 在ON24中建立您的網路研討會活動 — Marketo檔案 — 產品檔案
title: 在ON24中建立您的網路研討會活動
exl-id: a5211f11-c099-44a0-95eb-b43f10fa5c91
feature: Events
source-git-commit: 431bd258f9a68bbb9df7acf043085578d3d91b1f
workflow-type: tm+mt
source-wordcount: '170'
ht-degree: 0%

---

# 在ON24中建立您的網路研討會活動 {#create-your-webinar-event-in-on}

在ON24中建立您的網路研討會後，您可以建立Marketo活動並將其與ON24網路研討會建立關聯。 這可讓系統共用註冊和出勤資訊。 記下URL和其他資訊，以用於確認電子郵件和ICS檔案。

在建立您的ON24網路研討會中，有一些設定供Marketo和其他只供ON24使用的設定使用。 Marketo會使用這些設定：

**標題和說明**

* 事件標題 — 網路研討會的名稱。 您可在Marketo中檢視此內容。
* 事件摘要（選用） — 網路研討會的說明。 您可在Marketo中檢視此內容。

**日期與時間**

* 即時活動開始 — 開始日期和時間。 您可在Marketo中檢視此內容。
* 即時結束時間 — 完成日期和時間。 您可在Marketo中檢視此內容。

您的下一個步驟是 [在Marketo中建立事件](/help/marketo/product-docs/demand-generation/events/create-an-event/create-an-event-with-the-marketo-on24-adapter/create-an-event-in-marketo.md){target="_blank"}.

>[!MORELIKETHIS]
>
>[瞭解Marketo ON24介面卡事件](/help/marketo/product-docs/demand-generation/events/create-an-event/create-an-event-with-the-marketo-on24-adapter/understanding-marketo-on24-adapter-events.md){target="_blank"}
