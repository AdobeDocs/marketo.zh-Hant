---
description: 瞭解如何在Marketo中舉辦互動式網路研討會。 執行即時活動並使用參與工具與與會者聯絡。
title: 舉辦互動式網路研討會
...
1. 選取要顯示給早期加入者的版面。
   ...
   >[!NOTE]
   >
   >參與者最多可以在排程開始前15分鐘進入會議室，並看到使用中的版面配置。 建議為他們設計「大廳」配置。

   >[!TIP]
   >
   >啟用廣播控制以進入虛擬的綠色空間。 如此一來，主持人和主持人就能在不向會議室的參與者廣播音訊和視訊的情況下，私下交談和互相交談。 這是工作階段之前和之後的理想選擇，您可以在其中測試您的音訊和視訊，或在網路研討會之後進行彙報。
...
Broadcast Control為您的互動式網路研討會新增虛擬的綠色空間。 它可讓主持人和主持人私下交談和見面，而不需向會議室的參與者廣播。 在工作階段之前使用它測試您的麥克風和網路攝影機。 簡報者也可以在主持人和簡報者區域共同作業，直到他們準備好上線為止。 它提供一種方式，讓演講者和製作者在工作階段結束後私下互相彙報，以防與會者忘記離開網路研討會。
...
### 模擬的即時網路研討會 {#simulated-live-webinars}

呈現預先錄製的網路研討會，就像使用模擬即時網路研討會格式直播一樣。 與會者可在排程時間加入並即時體驗工作階段，享受聊天、投票和問答等互動功能。模擬的即時網路研討會結合已錄製內容的可靠性與即時事件的互動式體驗。

深入瞭解[模擬即時網路研討會](https://helpx.adobe.com/tw/adobe-connect/using/webinar/overview-of-simulated-live-webinars.html){target="_blank"}。
