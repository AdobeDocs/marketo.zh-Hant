...
* 隨選統計：此Widget提供下列資訊：
   * 隨選錄製可供檢視的天數：協助行銷人員執行動作，例如執行接近錄製30天可用性持續時間結束的電子郵件行銷活動。
   * 至今為止的隨選網路研討會訪客總數：至今為止觀看隨選錄製的所有不顯示註冊者人數。
   * 所有訪客的平均觀看持續時間（以分鐘為單位）：讓行銷人員瞭解已檢視的錄製內容量，以及可以使用哪些「智慧行銷活動」來鎖定超過特定觀看持續時間的銷售機會。
