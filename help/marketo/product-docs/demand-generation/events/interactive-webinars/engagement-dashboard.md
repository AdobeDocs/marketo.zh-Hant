...
儀表板會追蹤各種量度，包括投票回應、聊天活動和視訊互動，為參與者的偏好和行為提供有價值的深入分析。 它提供參與者的活動和參與層級的全面概觀，讓主機可監控即時出席率、參與率和互動。
...
此介面提供即時工作階段活動效能的概觀。 此概觀包含關鍵量度、一段時間內的參與度和參與者互動。 它可協助組織者評估活動是否成功，並找出需改善的領域。

從左側面板中選取「參與」 ，以檢視即時工作階段的效能。 選取參與摘要(PDF)以下載即時工作階段的效能摘要。 此概觀可分類為不同區段。

熒幕擷圖

**關鍵量度**

從關鍵量度檢視即時工作階段中的註冊數、出席者數和參與數。
...
從參與圖表瞭解即時工作階段期間一段時間的平均參與。 主機可監控參與程度如何波動，讓他們識別高互動或低互動的關鍵時刻。 檢查不同版面中參與參與參與者的平均參與分數有多少。
...
選取「移至互動」可檢視輪詢回應、QnA量度以及與會者在工作階段期間的回應。 從下節的「互動」和下載互動報表，檢視參與者在各個Pod的參與情形。

### 互動 {#interactions}

透過「互動」檢視出席者如何互動及參與工作階段。 追蹤對輪詢、QnA量度、出席者的回應以及相應Pod中捨棄的連結和檔案的回應。 主機也可以下載這些Pod的互動報表，以進行更佳的分析。 透過分析這些互動，找出趨勢並調整其策略，以鼓勵互動性更強且更吸引人的環境。

從左側面板中選取「互動」 ，以檢視出席者在Pod間的參與情形。

熒幕擷圖

從選單列選取[輪詢]索引標籤，檢視新增到投票的問題和回應。 此標籤會顯示下列資訊：

* 選取「輪詢報表(CSV)」 ，以下載輪詢面板中互動的報表。
* 輪詢問題及其型別。
* 回應數，以及輪詢Pod開啟並參與的時段。
* 選取[檢視全部`<number>`個回應]，在對話方塊視窗中檢視回應。

從選單列選取「其他互動」標籤，即可檢視其他Pod中的參與度並下載其報表。

熒幕擷圖

從右上角的下拉式清單中選取「下載互動報表」 ，即可下載不同Pod的報表。 從這裡下載QnA報表、連結和檔案報表，以及回應報表，以獲得更好的追蹤。
...
從左側面板中，選取隨選活動以檢視錄影摘要。 您也可以下載隨選活動的報表。
...
從左側面板中選取「參與者活動」，以檢視有關每個參與者參與層級的整合資訊。 檢視此參與如何從「參與者」參與層次分類至層次。 選取「使用者活動報表(CSV)」 ，以下載報表以提供更好的追蹤。
...
組織使用Adobe Connect提供高度客製化、品牌化、參與導向的網路研討會。 投票、聊天、問答和回應等互動工具有助於吸引出席者，並收集不僅僅是註冊和出席資料。 在與會者參與這些互動功能後，參與資料就會將與會者分類為三個參與層級：高、中及低。 行銷人員可使用參與層級來更新受眾區段、更新潛在客戶分數並警示銷售。
