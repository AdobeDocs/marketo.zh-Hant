---
description: 瞭解如何在Marketo中推廣互動式網路研討會。 透過電子郵件和行銷活動共用註冊連結及提高出席率。
title: 推廣互動式網路研討會
...
在註冊後的幾分鐘內，成員的「網路研討會URL」就會填入到「成員」標籤中。 如果您找不到網路研討會URL的欄，請確認該欄已新增至您的檢視。 這是每個註冊會員的個人化URL，可讓您在排程時間參加網路研討會，而不需要任何驗證。 內部交換的權杖會處理成員的驗證。
...
互動式網路研討會可讓您在網路研討會之前、期間或之後索取報名。 在所有情況下，您都需要與潛在客戶共用網路研討會URL。 在網路研討會開始前按一下連結，即會將連結傳送至網路研討會前登陸頁面。 在網路研討會期間按一下，系統就會將他們導向進行中的網路研討會。 在網路研討會後按一下，系統會將他們導向至網路研討會的錄影。

## 互動式網路研討會Token {#interactive-webinars-tokens}

使用權杖，在電子郵件中推廣互動式網路研討會，無需手動新增網路研討會詳細資訊。 這會改善整體效率，因為對網路研討會中繼資料所做的任何變更（例如網路研討會標題、開始日期等） 將會自動反映在您的資產中。
