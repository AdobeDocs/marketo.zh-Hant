...
# 發行說明：2023 年 7 月 {#release-notes-july-23}

下方提供2023年7月發行版本包含的所有功能。 請查看您的 Adobe Marketo Engage 版本是否提供這些功能。
...
* **Dynamic Chat的Chat Source**：「聊天」現在作為新人員記錄和活動的來源提供。 此來源可用於協助使用者篩選智慧清單/行銷活動中的活動。 也可以封鎖來源以停止欄位更新。
...
* **取消訂閱銷售Insight附加設定**：您現在可以透過取消訂閱附加的新設定選項，進一步控制取消訂閱訊息。 新功能可讓管理員控制何時何地以及在銷售電子郵件中出現哪些取消訂閱訊息。
...
* **銷售Insight動作的動態欄位提示**：您現在可以透過新增至Sales Insight動作中電子郵件範本的新欄位提示，提示銷售團隊進一步個人化電子郵件的特定部分。
...
* **銷售Insight動作的預設動態欄位**：預設動態欄位可讓您設定一個後援值，如果沒有可填入的值，動態欄位可解析成該值。 這有助於確保電子郵件發出並填入所有個人化。
...
* **銷售Insight的Dynamic Chat支援**：已進行更新，以支援此版本中為所有Dynamic Chat使用者引進的新Dynamic Chat屬性。
...
* **銷售Insight動作的Dynamic Chat支援**：我們現在可在銷售機會摘要中支援Dynamic Chat活動，以及在即時聊天路由給代理程式時的通知。
...
* **Dynamic Chat觸發權杖**：推出新的觸發權杖，供Dynamic Chat活動用於有趣的時刻。
...
* **升級至Google Analytics 4**： Marketo先前支援Universal Analytics，但將於2023年7月1日前停止記錄資料。 RTP使用者必須在其Google Analytics控制面板中遵循移轉程式，才能在RTP中完成對Google Analytics 4整合的支援。 此後，使用者在其登入頁面中將會有唯一的英數字元標籤（位於html的`<head>`區段內），可讓Marketo傳送資料給Google Analytics。
