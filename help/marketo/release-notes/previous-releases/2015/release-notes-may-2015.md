...
2015年5月發行版本包含下列功能。 請檢查您的Marketo版本是否有功能可用。 發行後，請務必回訪以尋找每個功能的詳細文章連結！

## 完全回應的登陸頁面

[完全回應的登陸頁面](/help/marketo/product-docs/demand-generation/landing-pages/guided-landing-pages/create-a-guided-landing-page.md)

我們將推出新的登入頁面編輯模式和範本語法。 與我們的「自由格式」登陸頁面編輯器不同，全新的「引導式」登陸頁面編輯器將為完全回應的登陸頁面提供結構化編輯體驗。
...
您在電子郵件程式準備好傳送前是否點選傳送？ 使用新的abort電子郵件程式按鈕來減速。 這將會直接停止飛行中的電子郵件計畫。

## 電子郵件傳遞能力  {#email-deliverability}

Marketo現在會在您新增的網域上每週執行自動化SPF和DKIM檢查。 檢查您的通知，隨時掌握最新資訊。

## 電子郵件範本行為變更 {#email-template-behavior-change}

自此版本起，現在允許有效的HTML註解，且在建立新電子郵件時不會將其刪除。
...
使用RTP的機器學習和預測性分析演演算法，向正確的潛在客戶建議正確的內容。 使用影像和文字說明，以視覺化方式增強您的內容資產，並建議多個內容資產。
