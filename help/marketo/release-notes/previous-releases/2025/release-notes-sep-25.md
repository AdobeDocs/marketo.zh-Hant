...
* **舊版銷售機會活動資料流淘汰**：在2026年6月底前，使用者必須從舊版銷售機會活動資料流移轉至新的Adobe I/O銷售機會活動資料流(LADS)。 此淘汰僅影響少數在與Adobe I/O Events整合前加入潛在客戶活動資料流的使用者。 如果您是更新的資料串流使用者，或已經透過I/O事件管理潛在客戶活動資料串流，這對您沒有任何影響。 請參閱[本文章](https://developer.adobe.com/events/docs/guides/using/marketo/marketo-lead-activity-data-stream-migration){target="_blank"}以瞭解移轉詳細資料。
