...
下方提供2022年8月發行版本包含的所有功能。 檢視您的Adobe Marketo Engage版本，瞭解是否有功能可用。

>[!AVAILABILITY]
>
>以星號(![星號](assets/yellow-star.png))為付費附加元件。 請聯絡您的Marketo Engage代表以瞭解更多資訊。

以下功能已於開始分階段推出 **2022年8月26日**.
...
* **Dynamic Chat聊天記錄**：檢視每次交談的聊天記錄，以深入瞭解每位網路訪客的興趣。
...
* **[更新下一代體驗的畫面](/help/marketo/product-docs/marketo-engage-modern-ux/toggle-switch.md){target="_blank"}**：我們將在新一代體驗中提供額外的重新整理畫面，透過切換開關提供更新的設計和可用性增強功能：
...
>[!NOTE]
>
>在行銷活動中將資產移至方案內的資料夾為例外情況。 此移動動作不會顯示資料夾階層，因為程式中的資料夾名稱不能重複。
...
* **Munchkin — 擴展的連結追蹤**：擴充對追蹤的支援 `tel` 和 `mailto` 使用Munchkin的連結來追蹤擴展的網頁行為集。

* **Webhook自訂標題可見性**：Webhook自訂標題現在顯示在「管理員> Webhook」標籤中，以便更佳顯示。

* **驗證碼**：評估表單提交的有效性 [使用reCAPTCHA v3](/help/marketo/product-docs/demand-generation/forms/using-captcha/enable-captcha-in-marketo-forms.md){target="_blank"} 為傳入的表單流量評分。 建立行銷工作流程以自動排除、隔離或刪除可疑的機器人流量。

* **核准表單的許可權**：此新增許可權可控制哪些設計人員可核准對其他Design Studio資產一致的表單變更。 這可防止其他設計人員在沒有擁有核准許可權的人審閱表單的情況下，將變更推送至表單。

* **一律在匿名合併後執行促銷活動重播**：匿名銷售機會合並發生在促銷活動重播之前，因此自訂欄位篩選器在匿名促銷活動重播完成時的行為會很可靠。

## 行銷資料環境 {#marketing-data-environment}

* **修正自訂物件「使用者」欄位的UI截斷**：現在更容易識別「使用中」的自訂物件欄位，因此您可以在必要時從自訂物件中刪除欄位。
...
* **[Sales Insight與Dynamic Chat整合](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/dynamic-chat-integration.md){target="_blank"}**：在Sales Insight面板中從Dynamic Chat檢視活動，並將這個新的資料點用於您的潛在客戶工作。
