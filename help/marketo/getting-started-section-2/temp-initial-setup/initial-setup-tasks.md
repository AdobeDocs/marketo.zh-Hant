...
# Marketo Engage的初始設定工作{#initial-setup-tasks-for-marketo-engage}

首次設定全新的Adobe Marketo Engage執行個體時，您必須執行幾項工作/強烈建議您先執行以啟動並執行。

進行初始設定後，請檢閱我們的 [新Marketo Engage例項的最佳作法](/help/marketo/temp-getting-started-section/temp-best-practice-checklists/where-to-start.md) 區段來充分利用您的投資，並為長期成功做好準備。

## 工作1：允許清單網域 {#allowlist-domains}
...
## 任務2：建立登入頁面和電子郵件的DNS記錄 {#step-create-dns-records-for-landing-pages-and-email}
...
## 工作3：允許列出Marketo IP {#step-allowlist-marketo-ips}
...
## 作業4：設定SPF與DKIM {#step-set-up-spf-and-dkim}
...
## 工作6：設定您網域的MX記錄 {#step-set-up-mx-records-for-your-domain}
